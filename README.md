...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            engine = manager.getEngineByExtension("etl");
...
可以通过Spring注入机制，来得到一个脚本引擎 `ScriptEngine` 实例对象。
...
```properties
{0}    如果正在执行脚本文件, {1} 表示脚本文件所在目录的绝对路径
{2}    如果正在执行脚本文件, {3} 表示脚本文件的名字
{4}    如果正在执行脚本文件, {5} 表示脚本文件的字符集编码
{6}    如果正在执行脚本文件, {7} 表示脚本文件中的行间分隔符
{8}    如果脚本引擎发生异常, {9} 表示异常详细信息
{10}    如果脚本引擎发生异常, {11} 表示报错的语句
{12}    如果脚本引擎发生数据库错误, {13} 表示数据库厂商定义的错误码
{14}    如果脚本引擎发生数据库错误, {15} 表示数据库厂商定义的SQL状态码
{16}    表示最近一次执行语句的返回值 
{17}    表示最近一次执行sql语句影响的数据记录数 
{18}    表示当前是否处于 jump 语句中. 如果 jump 变量等于 true 表示脚本引擎正处于 jump 语句中
{19}    表示当前步骤变量值，即最近一个 step 命令的参数值
{20}    表示临时文件所在目录
{21}    如果正在执行脚本文件, 表示当前脚本文件的绝对路径
{22}    局部变量名，用于存储当前数据库编目名
```
