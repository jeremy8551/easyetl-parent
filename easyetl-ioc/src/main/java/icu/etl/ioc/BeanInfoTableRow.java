...
public class BeanInfoTableRow extends ArrayList<BeanInfoRegister> {
     ...
     */
    public BeanInfoTableRow(Class<?> type) {
        super(10);
        this.type = type;
    }
     ...
     * @return 组件信息集合
     */
    public BeanInfoTableRow indexOf(String name) {
        if (StringUtils.isBlank(name)) {
            return this;
        }

        BeanInfoTableRow list = new BeanInfoTableRow(this.type);
        for (int i = 0, size = this.size(); i < size; i++) {
    ...
    public BeanInfoTableRow indexOf(BeanInfoFilter filter) {
        BeanInfoTableRow list = new BeanInfoTableRow(this.type);
        for (int i = 0, size = this.size(); i < size; i++) {
            BeanInfoRegister beanInfo = this.get(i);
    ...
    public static String toString(List<? extends BeanInfo> list) {
        ...
        for (BeanInfo beanInfo : list) {
            ...
            if (beanInfo instanceof BeanInfoCell) {
                ct.addCell(((BeanInfoCell) beanInfo).getBean());
            }
        }
