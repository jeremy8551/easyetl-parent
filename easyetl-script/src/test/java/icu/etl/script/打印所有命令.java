...
public class 打印所有命令 {
    ...
    public static void main(String[] args) throws IOException {
        BeanContext context = new BeanContext("sout:info");
        ...
        Collections.sort(commands, new Comparator<BeanConfig>() {
            public int compare(BeanConfig o1, BeanConfig o2) {
                return o1.getImplementClass().getName().compareTo(o2.getImplementClass().getName());
            }
        });
        ...
        for (BeanConfig cls : commands) {
            STD.out.info(cls.getImplementClass().getName());
            buf.append("\t\t").append("this.loadScriptCommand(" + cls.getImplementClass().getName() + ".class);").append(FileUtils.lineSeparator);
        }
//		Std.out.info("共找到 " + commands.size() + " 个脚本命令类, 将以上内容替换到 " + CommandScanner.class.getName() + ".loadCommandBuilder() 方法中");
        ...
        for (BeanConfig cls : methods) {
            STD.out.info(cls.getImplementClass().getName());
            buf.append("\t\t").append("this.loadVariableMethod(" + cls.getImplementClass().getName() + ".class);").append(FileUtils.lineSeparator);
        }
