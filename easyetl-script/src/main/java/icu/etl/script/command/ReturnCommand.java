...
public class ReturnCommand extends AbstractSlaveCommand implements UniversalScriptInputStream, LoopCommandKind {
    ...
    public void read(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptParser parser, UniversalScriptAnalysis analysis, Reader in) throws IOException {
        if (analysis.isBlankline(this.returnCode)) {
            ...
            throw new UniversalScriptException(ResourcesUtils.getScriptStderrMessage(14, this.command, "return", this.returnCode));
        }
    }
    ...
    public int execute(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptStdout stdout, UniversalScriptStderr stderr, boolean forceStdout, File outfile, File errfile) throws Exception {
        if (this.existsOwner()) {
            ...
            if (value.length() == 0) { // 如果没有返回值默认使用最近一次命名是否执行成功作为退出值
                stderr.println(ResourcesUtils.getScriptStderrMessage(16));
                ...
                stderr.println(ResourcesUtils.getScriptStderrMessage(17, value));
                return UniversalScriptCommand.COMMAND_ERROR;
            }
        } else {
            stderr.println(ResourcesUtils.getScriptStderrMessage(15));
            return UniversalScriptCommand.COMMAND_ERROR;
        }
    }
