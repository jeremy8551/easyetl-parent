...
`{53}.keywords()` 返回关键字数组（关键字不能作为变量名使用）。 
...
脚本引擎启动时，自动扫描 `{0}` 包下的类，可通过配置下面这个参数，可以扫描特定的包下的类。

```java
System.setProperty("{70}", "icu.etl");
```

程序启动时，默认使用Slf4j作为日志输出接口，如果工程中未使用Slf4j，则默认使用System.out作为日志输出，可以通过如下参数配置
