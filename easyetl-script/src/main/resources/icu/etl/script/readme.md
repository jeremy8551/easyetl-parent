...
简化脚本引擎在 **Springboot** 项目中的使用，引用脚本引擎的场景启动器后，无需额外配置，可直接使用。
...
脚本引擎容器是脚本引擎的基础功能，类似于Spring容器的概念，主要用来管理系统内部组件与外部注册组件。
...
当内部容器启动执行初始化操作时，会使用类扫描器扫描 `{34}` 包下的类文件，将扫描到的类信息存储到**组件信息表**中。

类扫描器的实现类是 `{107}`，扫描规则详见 `{108}` 接口的实现类，可通过如下参数指定扫描规则：

```java
System.setProperty("{70}", "{0},!org.apache"); // 包名前面的叹号，表示扫描时会排除这个包名下的所有类
```

类扫描时默认只会扫描注解 `{22}`、`{51}`、`{53}` 标记的类，如果想增加扫描规则，则可以在**SPI**配置文件 `resources/META-INF/services/{102}` 中增加 `{108}` 接口的实现类。
...
脚本引擎编译器接口是 `{109}`，可以将一段脚本语句解析编译为可以执行的脚本命令。
...
脚本引擎产生的临时文件，默认存储在 `${java.io.tmpdir}/{35}` 路径下，可通过如下参数修改默认路径：
...
这样我们就可以通过一个统一一致的数据库方言接口 `{26}`，来操作不同品牌的数据库。
