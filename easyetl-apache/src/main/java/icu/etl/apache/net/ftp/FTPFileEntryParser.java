 ...
 * <p>
 * The first example shows how to get an <b>iterable</b> list of files in which the more expensive <code>FTPFile</code> objects are not created until needed. This is suitable for paged displays. It requires that a parser object be created beforehand: <code>parser</code> is an object (in the package <code>org.apache.commons.net.ftp.parser</code>) implementing this inteface.
 *
 * <pre>
 ...
 * </pre>
 *
 * The second example uses the revised <code>FTPClient.listFiles()</code> API to pull the whole list from the subfolder <code>subfolder</code> in one call, attempting to automatically detect the parser type. This method, without a parserKey parameter, indicates that autodection should be used.
 *
 * <pre>
 * FTPClient f = FTPClient();
 * f.connect(server);
 ...
 * </pre>
 *
 * The third example uses the revised <code>FTPClient.listFiles()</code>> API to pull the whole list from the current working directory in one call, but specifying by classname the parser to be used. For this particular parser class, this approach is necessary since there is no way to autodetect this server type.
 *
 * <pre>
 * FTPClient f = FTPClient();
 * f.connect(server);
 ...
 * </pre>
 *
 * The fourth example uses the revised <code>FTPClient.listFiles()</code> API to pull a single file listing in an arbitrary directory in one call, specifying by KEY the parser to be used, in this case, VMS.
 *
 * <pre>
 * FTPClient f = FTPClient();
 * f.connect(server);
...
public interface FTPFileEntryParser {
	/**
	 * Parses a line of an FTP server file listing and converts it into a usable format in the form of an <code> FTPFile </code> instance. If the file listing line doesn't describe a file, <code> null </code> should be returned, otherwise a <code> FTPFile </code> instance representing the files in the directory is returned.
	 * <p>
	 * 
	 * @param listEntry
