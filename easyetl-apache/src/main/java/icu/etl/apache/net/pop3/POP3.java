...
/***
 * The POP3 class is not meant to be used by itself and is provided only so that you may easily implement your own POP3 client if you so desire. If you have no need to perform your own implementation, you should use {@link POP3Client}.
 * <p>
...
public class POP3 extends SocketClient {
	...
	/***
	 * Sets POP3 client state. This must be one of the <code>_STATE</code> constants.
	 * <p>
	 * 
	 * @param state
	 *            The new state.
	...
	/***
	 * Disconnects the client from the server, and sets the state to <code> DISCONNECTED_STATE </code>. The reply text information from the last issued command is voided to allow garbage collection of the memory used to store that information.
	 * <p>
	 * 
	...
	/***
	 * Returns an array of lines received as a reply to the last command sent to the server. The lines have end of lines truncated. If the reply is a single line, but its format ndicates it should be a multiline reply, then you must call {@link #getAdditionalReply getAdditionalReply() } to fetch the rest of the reply, and then call <code>getReplyStrings</code> again. You only have to worry about this if you are implementing your own client using the {@link #sendCommand sendCommand } methods.
	 * <p>
	 * 
	...
	/***
	 * Returns the reply to the last command sent to the server. The value is a single string containing all the reply lines including newlines. If the reply is a single line, but its format ndicates it should be a multiline reply, then you must call {@link #getAdditionalReply getAdditionalReply() } to fetch the rest of the reply, and then call <code>getReplyString</code> again. You only have to worry about this if you are implementing your own client using the {@link #sendCommand sendCommand } methods.
	 * <p>
	 * 
	 * @return The last server response.
	 ***/
