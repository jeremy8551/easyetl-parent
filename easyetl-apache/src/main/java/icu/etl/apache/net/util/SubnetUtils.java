 ...
 */
package icu.etl.apache.net.util;
...
public class SubnetUtils {
	...
	public final class SubnetInfo {
		...
		private int low() {
			return network() + (isInclusiveHostCount() ? 0 : 1);
		}
		
		private int high() {
			return broadcast() - (isInclusiveHostCount() ? 0 : 1);
		}
		
		/**
		 * Returns true if the parameter <code>address</code> is in the range of usable endpoint addresses for this subnet. This excludes the network and broadcast adresses.
		 * 
		 * @param address
		...
		public int getAddressCount() {
			return (broadcast() - low() + (isInclusiveHostCount() ? 1 : 0));
		}
