...
/***
 * RLoginClient is very similar to {@link RCommandClient}, from which it is derived, and uses the rcmd() facility implemented in RCommandClient to implement the functionality of the rlogin command that first appeared in 4.2BSD Unix. rlogin is a command used to login to a remote machine from a trusted host, sometimes without issuing a password. The trust relationship is the same as described in the documentation for {@link RCommandClient}.
 * <p>
 * As with virtually all of the client classes in org.apache.commons.net, this class derives from SocketClient. But it relies on the connection methods defined in RcommandClient which ensure that the local Socket will originate from an acceptable rshell port. The way to use RLoginClient is to first connect to the server, call the {@link #rlogin rlogin() } method, and then fetch the connection's input and output streams. Interaction with the remote command is controlled entirely through the I/O streams. Once you have finished processing the streams, you should invoke {@link RExecClient#disconnect disconnect() } to clean up properly.
 * <p>
 * The standard output and standard error streams of the remote process are transmitted over the same connection, readable from the input stream returned by {@link RExecClient#getInputStream getInputStream() } . Unlike RExecClient and RCommandClient, it is not possible to tell the rlogind daemon to return the standard error stream over a separate connection. {@link RExecClient#getErrorStream getErrorStream() } will always return null. The standard input of the remote process can be written to through the output stream returned by {@link RExecClient#getOutputStream getOutputSream() } .
 * <p>
 * <p>
 * 
...
public class RLoginClient extends RCommandClient {
	 ...
	 * 
	 * @param localUsername
	 *            The user account on the local machine that is trying to login to the remote host.
	 * @param remoteUsername
	 *            The account name on the server that is being logged in to.
	 * @param terminalType
	 *            The name of the user's terminal (e.g., "vt100", "network", etc.)
	 * @param terminalSpeed
	 *            The speed of the user's terminal, expressed as a baud rate or bps (e.g., 9600 or 38400)
