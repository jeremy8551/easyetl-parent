...
/***
 * SMTP provides the basic the functionality necessary to implement your own SMTP client. To derive the full benefits of the SMTP class requires some knowledge of the FTP protocol defined in RFC 821. However, there is no reason why you should have to use the SMTP class. The {@link SMTPClient} class, derived from SMTP, implements all the functionality required of an SMTP client. The SMTP class is made public to provide access to various SMTP constants and to make it easier for adventurous programmers (or those with special needs) to interact with the SMTP protocol and implement their own clients. A set of methods with names corresponding to the SMTP command names are provided to facilitate this interaction.
 * <p>
 * You should keep in mind that the SMTP server may choose to prematurely close a connection for various reasons. The SMTP class will detect a premature SMTP server connection closing when it receives a {@link SMTPReply#SERVICE_NOT_AVAILABLE SMTPReply.SERVICE_NOT_AVAILABLE } response to a command. When that occurs, the SMTP class method encountering that reply will throw an {@link SMTPConnectionClosedException} . <code>SMTPConectionClosedException</code> is a subclass of <code> IOException </code> and therefore need not be caught separately, but if you are going to catch it separately, its catch block must appear before the more general <code> IOException </code> catch block. When you encounter an {@link SMTPConnectionClosedException} , you must disconnect the connection with {@link SocketClient#disconnect disconnect() } to properly clean up the system resources used by SMTP.
 * Before disconnecting, you may check the last reply code and text with {@link #getReplyCode getReplyCode }, {@link #getReplyString getReplyString }, and {@link #getReplyStrings getReplyStrings}.
 * <p>
...
public class SMTP extends SocketClient {
	...
	/***
	 * The default SMTP constructor. Sets the default port to <code>DEFAULT_PORT</code> and initializes internal data structures for saving SMTP reply information.
	 ***/
	public SMTP() {
	...
	/***
	 * Returns the integer value of the reply code of the last SMTP reply. You will usually only use this method after you connect to the SMTP server to check that the connection was successful since <code> connect </code> is of type void.
	 * <p>
	 * 
