 ...
 * <p>
 * The second part is composed of the actual parsing, again in conjunction with the particular parser used by this engine. This is controlled by an iterator over the internal list of strings. This may be done either in block mode, by calling the <code>getNext()</code> and <code>getPrevious()</code> methods to provide "paged" output of less than the whole list at one time, or by calling the <code>getFiles()</code> method to return the entire list.
 * <p>
 * Examples:
 * <p>
 * Paged access:
...
//	}
	...
	/**
	 * Returns an array of at most <code>quantityRequested</code> FTPFile objects starting at this object's internal iterator's current position. If fewer than <code>quantityRequested</code> such elements are available, the returned array will have a length equal to the number of entries at and after after the current position. If no such entries are found, this array will have a length of 0.
	 *
	 ...
	 *		   
	 * @return an array of at most <code>quantityRequested</code> FTPFile objects starting at the current position of this iterator within its list and at least the number of elements which exist in the list at and after its current position.
	 *         <p>
	 ...
	 *
	 * If fewer than <code>quantityRequested</code> such elements are available, the returned array will have a length equal to the number of entries at and after after the current position. If no such entries are found, this array will have a length of 0.
	 *
	 ...
	 *            the maximum number of entries we want to get.
	 *		   
	 * @return an array of at most <code>quantityRequested</code> FTPFile objects starting at the current position of this iterator within its list and at least the number of elements which exist in the list at and after its current position. This array will be in the same order as the underlying list (not reversed).
	 *         <p>
	 *         <b> NOTE:</b> This array may contain null members if any of the individual file listings failed to parse. The caller should check each entry for null before referencing it.
	 */
	 ...
	 * @return an array of FTPFile objects containing the whole list of files returned by the server as read by this object's parser.
	 *         <p>
	 *         <b> NOTE:</b> This array may contain null members if any of the individual file listings failed to parse. The caller should check each entry for null before referencing it, or use the a filter such as {@link FTPFileFilters#NON_NULL} which does not allow null entries.
	 * @since 2.2
	 * @exception IOException
	 */
