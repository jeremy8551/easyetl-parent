...
package icu.etl.apache.net.smtp;
...
/***
 * SMTPClient encapsulates all the functionality necessary to send files through an SMTP server. This class takes care of all low level details of interacting with an SMTP server and provides a convenient higher level interface. As with all classes derived from {@link SocketClient}, you must first connect to the server with {@link SocketClient#connect connect } before doing anything, and finally {@link SocketClient#disconnect disconnect } after you're completely finished interacting with the server. Then you need to check the SMTP reply code to see if the connection was successful. For example:
 * 
 * <pre>
 *    try {
 *      int reply;
 ...
 * </pre>
 * <p>
 * Immediately after connecting is the only real time you need to check the reply code (because connect is of type void). The convention for all the SMTP command methods in SMTPClient is such that they either return a boolean value or some other value. The boolean methods return true on a successful completion reply from the SMTP server and false on a reply resulting in an error condition or failure. The methods returning a value other than boolean return a value containing the higher level data produced by the SMTP command, or null if a reply resulted in an error condition or failure. If you want to access the exact SMTP reply code causing a success or failure, you must call {@link SMTP#getReplyCode getReplyCode } after a success or failure.
 * <p>
 * You should keep in mind that the SMTP server may choose to prematurely close a connection for various reasons. The SMTPClient class will detect a premature SMTP server connection closing when it receives a {@link SMTPReply#SERVICE_NOT_AVAILABLE SMTPReply.SERVICE_NOT_AVAILABLE } response to a command. When that occurs, the method encountering that reply will throw an {@link SMTPConnectionClosedException} . <code>SMTPConectionClosedException</code> is a subclass of <code> IOException </code> and therefore need not be caught separately, but if you are going to catch it separately, its catch block must appear before the more general <code> IOException </code> catch block. When you encounter an {@link SMTPConnectionClosedException} , you must disconnect the connection with {@link #disconnect disconnect() } to properly clean up the system resources used by SMTPClient. Before disconnecting, you may check
 * the last reply code and text with {@link SMTP#getReplyCode getReplyCode }, {@link SMTP#getReplyString getReplyString }, and {@link SMTP#getReplyStrings getReplyStrings}.
 * <p>
...
public class SMTPClient extends SMTP {
	...
	/***
	 * At least one SMTPClient method ({@link #sendMessageData sendMessageData }) does not complete the entire sequence of SMTP commands to complete a transaction. These types of commands require some action by the programmer after the reception of a positive intermediate command. After the programmer's code completes its actions, it must call this method to receive the completion reply from the server and verify the success of the entire transaction.
	 * <p>
	 * For example,
	 * 
	 * <pre>
	 ...
	 *                If the SMTP server prematurely closes the connection as a result of the client being idle or some other reason causing the server to send SMTP reply code 421. This exception may be caught either as an IOException or independently as itself.
	...
	/***
	 * A convenience method for a sending short email without having to explicitly set the sender and recipient(s). This method sets the sender and recipient using {@link #setSender setSender } and {@link #addRecipient addRecipient }, and then sends the message using {@link #sendShortMessageData sendShortMessageData }.
	 * <p>
	 * 
	 * @param sender
	...
	/***
	 * A convenience method for a sending short email without having to explicitly set the sender and recipient(s). This method sets the sender and recipients using {@link #setSender setSender } and {@link #addRecipient addRecipient }, and then sends the message using {@link #sendShortMessageData sendShortMessageData }.
	 * <p>
	 * 
	 * @param sender
	 *            The email address of the sender.
