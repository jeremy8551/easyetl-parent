...
public class NtpV3Impl implements NtpV3Packet {
	...
	public static final int KEY_IDENTIFIER_INDEX = 48;
	public static final int MESSAGE_DIGEST = 54; /* len 16 bytes */
	...
	private long getLong(int index) {
		long i = ul(buf[index]) << 56 | ul(buf[index + 1]) << 48 | ul(buf[index + 2]) << 40 | ul(buf[index + 3]) << 32 | ul(buf[index + 4]) << 24 | ul(buf[index + 5]) << 16 | ul(buf[index + 6]) << 8 | ul(buf[index + 7]);
		return i;
	}
