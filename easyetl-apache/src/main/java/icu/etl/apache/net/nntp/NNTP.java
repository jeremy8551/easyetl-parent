...
/***
 * The NNTP class is not meant to be used by itself and is provided only so that you may easily implement your own NNTP client if you so desire. If you have no need to perform your own implementation, you should use {@link NNTPClient}. The NNTP class is made public to provide access to various NNTP constants and to make it easier for adventurous programmers (or those with special needs) to interact with the NNTP protocol and implement their own clients. A set of methods with names corresponding to the NNTP command names are provided to facilitate this interaction.
 * <p>
 ...
 * {@link #disconnect disconnect() } to properly clean up the system resources used by NNTP. Before disconnecting, you may check the last reply code and text with {@link #getReplyCode getReplyCode } and {@link #getReplyString getReplyString }.
 * <p>
...
public class NNTP extends SocketClient {
	...
	/***
	 * The default NNTP constructor. Sets the default port to <code>DEFAULT_PORT</code> and initializes internal data structures for saving NNTP reply information.
	 ***/
	public NNTP() {
	 ...
	 * @param args
	 *            The arguments to the NNTP command. If this parameter is set to null, then the command is sent with no argument.
	 ...
	 * 
	 * @param command
	 *            The NNTPCommand constant corresponding to the NNTP command to send.
	 * @param args
	 *            The arguments to the NNTP command. If this parameter is set to null, then the command is sent with no argument.
	...
	/***
	 * Returns the integer value of the reply code of the last NNTP reply. You will usually only use this method after you connect to the NNTP server to check that the connection was successful since <code> connect </code> is of type void.
	 * <p>
	 * 
	 ...
	 * 
	 * @param messageId
	 *            The message identifier of the requested article, including the encapsulating &lt and &gt characters.
	 ...
	 * 
	 * @param articleNumber
	 *            The number of the article to request from the currently selected newsgroup.
	 * @return The reply code received from the server.
	 * @exception NNTPConnectionClosedException
	 *                If the NNTP server prematurely closes the connection as a result of the client being idle or some other reason causing the server to send NNTP reply code 400. This exception may be caught either as an IOException or independently as itself.
	 * @exception IOException
	 ...
	 * 
	 * @param messageId
	 *            The article identifier, including the encapsulating &lt and &gt characters.
	 * @return The reply code received from the server.
	 * @exception NNTPConnectionClosedException
	 *                If the NNTP server prematurely closes the connection as a result of the client being idle or some other reason causing the server to send NNTP reply code 400. This exception may be caught either as an IOException or independently as itself.
	 * @exception IOException
	 ...
	 * 
	 * @param date
	 *            The date after which to check for new groups. Date format is YYMMDD
	 * @param time
	 *            The time after which to check for new groups. Time format is HHMMSS using a 24-hour clock.
	 * @param GMT
	 *            True if the time is in GMT, false if local server time.
	 * @param distributions
	 *            Comma-separated distribution list to check for new groups. Set to null if no distributions.
	 * @return The reply code received from the server.
	 * @exception NNTPConnectionClosedException
	 *                If the NNTP server prematurely closes the connection as a result of the client being idle or some other reason causing the server to send NNTP reply code 400. This exception may be caught either as an IOException or independently as itself.
	 * @exception IOException
	 ...
	 * @param date
	 *            The date after which to check for new news. Date format is YYMMDD
	 * @param time
	 *            The time after which to check for new news. Time format is HHMMSS using a 24-hour clock.
	 * @param GMT
	 *            True if the time is in GMT, false if local server time.
	 * @param distributions
	 *            Comma-separated distribution list to check for new news. Set to null if no distributions.
	 * @return The reply code received from the server.
	 * @exception NNTPConnectionClosedException
	 *                If the NNTP server prematurely closes the connection as a result of the client being idle or some other reason causing the server to send NNTP reply code 400. This exception may be caught either as an IOException or independently as itself.
	 * @exception IOException
	 ...
	 * 
	 * @param header
	 *            a String naming a header line (e.g., "subject"). See RFC-1036 for a list of valid header lines.
	 * @param selectedArticles
	 *            a String representation of the range of article headers required. This may be an article number, or a range of article numbers in the form "XXXX-YYYY", where XXXX and YYYY are valid article numbers in the current group. It also may be of the form "XXX-", meaning "return XXX and all following articles" In this revision, the last format is not possible (yet).
	 * @return The reply code received from the server.
	 * @exception NNTPConnectionClosedException
