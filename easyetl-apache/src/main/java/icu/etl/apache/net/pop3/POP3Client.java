...
package icu.etl.apache.net.pop3;
...
public class POP3Client extends POP3 {
	...
	/***
	 * Login to the POP3 server with the given username and password. You must first connect to the server with {@link SocketClient#connect connect } before attempting to login. A login attempt is only valid if the client is in the {@link POP3#AUTHORIZATION_STATE AUTHORIZATION_STATE } . After logging in, the client enters the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } .
	 * <p>
	 * 
	 * @param username
	...
	/***
	 * Login to the POP3 server with the given username and authentication information. Use this method when connecting to a server requiring authentication using the APOP command. Because the timestamp produced in the greeting banner varies from server to server, it is not possible to consistently extract the information. Therefore, after connecting to the server, you must call {@link POP3#getReplyString getReplyString } and parse out the timestamp information yourself.
	 * <p>
	 * You must first connect to the server with {@link SocketClient#connect connect } before attempting to login. A login attempt is only valid if the client is in the {@link POP3#AUTHORIZATION_STATE AUTHORIZATION_STATE } . After logging in, the client enters the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } . After connecting, you must parse out the server specific information to use as a timestamp, and pass that information to this method. The secret is a shared secret known to you and the server. See RFC 1939 for more details regarding the APOP command.
	 * <p>
	 * 
	 * @param username
	 *            The account name being logged in to.
	 ...
	 * @param secret
	 *            The shared secret which produces the MD5 digest when combined with the timestamp.
	 * @return True if the login attempt was successful, false if not.
	 * @exception IOException
	 *                If a network I/O error occurs in the process of logging in.
	...
	/***
	 * Logout of the POP3 server. To fully disconnect from the server you must call {@link POP3#disconnect disconnect }. A logout attempt is valid in any state. If the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } , it enters the {@link POP3#UPDATE_STATE UPDATE_STATE } on a successful logout.
	 * <p>
	 * 
	...
	/***
	 * Send a NOOP command to the POP3 server. This is useful for keeping a connection alive since most POP3 servers will timeout after 10 minutes of inactivity. A noop attempt will only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } .
	 * <p>
	 * 
	...
	/***
	 * Reset the POP3 session. This is useful for undoing any message deletions that may have been performed. A reset attempt can only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } .
	 * <p>
	 * 
	...
	/***
	 * Get the mailbox status. A status attempt can only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } . Returns a POP3MessageInfo instance containing the number of messages in the mailbox and the total size of the messages in bytes. Returns null if the status the attempt fails.
	 * <p>
	 * 
	 * @return A POP3MessageInfo instance containing the number of messages in the mailbox and the total size of the messages in bytes. Returns null if the status the attempt fails.
	 * @exception IOException
	 ...
	 * @return A POP3MessageInfo instance containing the number of the listed message and the size of the message in bytes. Returns null if the list attempt fails.
	 * @exception IOException
	...
	/***
	 * List all messages. A list attempt can only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } . Returns an array of POP3MessageInfo instances, each containing the number of a message and its size in bytes. If there are no messages, this method returns a zero length array. If the list attempt fails, it returns null.
	 * <p>
	 * 
	 * @return An array of POP3MessageInfo instances representing all messages in the order they appear in the mailbox, each containing the number of a message and its size in bytes. If there are no messages, this method returns a zero length array. If the list attempt fails, it returns null.
	 * @exception IOException
	 *                If a network I/O error occurs in the process of sending the list command.
	 ***/
	...
	/***
	 * List the unique identifier for a message. A list attempt can only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } . Returns a POP3MessageInfo instance containing the number of the listed message and the unique identifier for that message. Returns null if the list attempt fails (e.g., if the specified message number does not exist).
	 * <p>
	 * 
	 * @param messageId
	 *            The number of the message list.
	 * @return A POP3MessageInfo instance containing the number of the listed message and the unique identifier for that message. Returns null if the list attempt fails.
	 * @exception IOException
	...
	/***
	 * List the unique identifiers for all messages. A list attempt can only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } . Returns an array of POP3MessageInfo instances, each containing the number of a message and its unique identifier. If there are no messages, this method returns a zero length array. If the list attempt fails, it returns null.
	 * <p>
	 * 
	 * @return An array of POP3MessageInfo instances representing all messages in the order they appear in the mailbox, each containing the number of a message and its unique identifier If there are no messages, this method returns a zero length array. If the list attempt fails, it returns null.
	 * @exception IOException
	 *                If a network I/O error occurs in the process of sending the list unique identifier command.
	 ***/
	 ...
	 * @return A DotTerminatedMessageReader instance from which the entire message can be read. Returns null if the retrieval attempt fails (e.g., if the specified message number does not exist).
	 * @exception IOException
	...
	/***
	 * Retrieve only the specified top number of lines of a message from the POP3 server. A retrieve top lines attempt can only succeed if the client is in the {@link POP3#TRANSACTION_STATE TRANSACTION_STATE } . Returns a DotTerminatedMessageReader instance from which the specified top number of lines of the message can be read. Returns null if the retrieval attempt fails (e.g., if the specified message number does not exist).
	 * <p>
	 * You must not issue any commands to the POP3 server (i.e., call any other methods) until you finish reading the message from the returned Reader instance. The POP3 protocol uses the same stream for issuing commands as it does for returning results. Therefore the returned Reader actually reads directly from the POP3 connection. After the end of message has been reached, new commands can be executed and their replies read. If you do not follow these requirements, your program will not work properly.
	 * <p>
	 * 
	 ...
	 * @return A DotTerminatedMessageReader instance from which the specified top number of lines of the message can be read. Returns null if the retrieval attempt fails (e.g., if the specified message number does not exist).
	 * @exception IOException
