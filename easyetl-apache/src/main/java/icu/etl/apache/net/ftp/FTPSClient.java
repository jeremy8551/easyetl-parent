...
/**
 * FTP over SSL processing. If desired, the JavaUtils property -Djavax.net.debug=all can be used to see wire-level SSL details.
 *
...
//	public static String STORE_TYPE;
	 ...
	 *             A requested cryptographic algorithm is not available in the environment.
	 */
	...
	private void execAUTH() throws SSLException, IOException {
		int replyCode = sendCommand(FTPSCommand._commands[FTPSCommand.AUTH], auth);
	...
	/**
	 * Returns true if new SSL sessions may be established by this socket. When the underlying {@link Socket} instance is not SSL-enabled (i.e. an instance of {@link SSLSocket} with {@link SSLSocket}{@link #getEnableSessionCreation()}) enabled, this returns False.
	 * 
	...
	/**
	 * Returns true if the socket is set to use client mode in its first handshake. When the underlying {@link Socket} is not an {@link SSLSocket} instance, returns false.
	 * 
	...
	/**
	 * Returns the names of the cipher suites which could be enabled for use on this connection. When the underlying {@link Socket} is not an {@link SSLSocket} instance, returns null.
	 * 
	...
	/**
	 * Returns the names of the protocol versions which are currently enabled for use on this connection. When the underlying {@link Socket} is not an {@link SSLSocket} instance, returns null.
	 * 
	...
	public void execPBSZ(long pbsz) throws SSLException, IOException {
		...
		if (FTPReply.COMMAND_OK != sendCommand(FTPSCommand._commands[FTPSCommand.PBSZ], String.valueOf(pbsz)))
			throw new SSLException(getReplyString());
	}
	...
	public void execPROT(String prot) throws SSLException, IOException {
		...
		if (!checkPROTValue(prot))
			throw new IllegalArgumentException();
		if (FTPReply.COMMAND_OK != sendCommand(FTPSCommand._commands[FTPSCommand.PROT], prot))
			throw new SSLException(getReplyString());
	...
	/**
	 * Send an FTP command. The CCC (Clear Command Channel) command causes the underlying {@link SSLSocket} instance to be assigned to a plain {@link Socket} instances
	 * 
	 * @param command
	 *            The FTP command.
	 * @return server reply.
	 ...
	 * @return corresponding to the established data connection. Null is returned if an FTP protocol error is reported at any point during the establishment and initialization of the connection.
	 * @throws IOException
