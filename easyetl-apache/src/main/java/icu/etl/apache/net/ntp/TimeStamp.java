package icu.etl.apache.net.ntp;
/*
...
public class TimeStamp implements java.io.Serializable, Comparable<TimeStamp> {
	...
	/*
	 * static { TimeZone utcZone = TimeZone.getTimeZone("UTC"); Calendar calendar = Calendar.getInstance(utcZone); calendar.set(1900, Calendar.JANUARY, 1, 0, 0, 0); calendar.set(Calendar.MILLISECOND, 0); msb1baseTime = calendar.getTime().getTime(); calendar.set(2036, Calendar.FEBRUARY, 7, 6, 28, 16); calendar.set(Calendar.MILLISECOND, 0); msb0baseTime = calendar.getTime().getTime(); }
	 */
	...
	public static long getTime(long ntpTimeValue) {
		 ...
		 *
		 * If bit 0 is set, the UTC time is in the range 1968-2036 and UTC time is reckoned from 0h 0m 0s UTC on 1 January 1900. If bit 0 is not set, the time is in the range 2036-2104 and UTC time is reckoned from 6h 28m 16s UTC on 7 February 2036.
		 */
	...
	/***
	 * Computes a hashcode for this Timestamp. The result is the exclusive OR of the two halves of the primitive <code>long</code> value represented by this <code>TimeStamp</code> object. That is, the hashcode is the value of the expression: <blockquote>
	 * 
	 * <pre>
	...
	/***
	 * Compares this object against the specified object. The result is <code>true</code> if and only if the argument is not <code>null</code> and is a <code>Long</code> object that contains the same <code>long</code> value as this object.
	 *
	 * @param obj
	...
	/***
	 * Converts this <code>TimeStamp</code> object to a <code>String</code>. The NTP timestamp 64-bit long value is represented as hex string with seconds separated by fractional seconds by a decimal point; e.g. c1a089bd.fc904f6d <=> Tue, Dec 10 2002 10:41:49.986
	 *
	...
	/***
	 * Converts 64-bit NTP timestamp value to a <code>String</code>. The NTP timestamp value is represented as hex string with seconds separated by fractional seconds by a decimal point; e.g. c1a089bd.fc904f6d <=> Tue, Dec 10 2002 10:41:49.986
	 *
	 * @return NTP timestamp 64-bit long value as hex string with seconds separated by fractional seconds.
	 */
	 ...
	 * @return the value <code>0</code> if the argument TimeStamp is equal to this TimeStamp; a value less than <code>0</code> if this TimeStamp is numerically less than the TimeStamp argument; and a value greater than <code>0</code> if this TimeStamp is numerically greater than the TimeStamp argument (signed comparison).
	 */
