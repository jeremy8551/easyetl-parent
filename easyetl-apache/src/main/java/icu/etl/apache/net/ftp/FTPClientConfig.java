...
import icu.etl.apache.net.ftp.parser.ConfigurableFTPFileEntryParserImpl;
import icu.etl.apache.net.ftp.parser.FTPTimestampParserImpl;
 ...
 * </p>
 * <h3>Examples of use of FTPClientConfig</h3> Use cases: You are trying to access a server that
 * <ul>
 ...
 * <li>is in different timezone and you need accurate timestamps for dependency checking as in Ant</li>
 * </ul>
 * <p>
 ...
 * </p>
 * <p>
 * Unpaged (whole list) access on a VMS server that uses month names in a language not {@link #getSupportedLanguageCodes() supported} by the system. but uses the "standard" <code>MMM d yyyy</code> date formatting
 * 
 * <pre>
 * FTPClient f = FTPClient();
...
public class FTPClientConfig {
	...
	/**
	 * Some servers return an "UNKNOWN Type: L8" message in response to the SYST command. We set these to be a Unix-type system. This may happen if the ftpd in question was compiled without system information.
	 *
	 ...
	 * 
	 * @param systemKey
	 *            key representing system type of the server being connected to. See {@link #getServerSystemKey() serverSystemKey}
	 */
	 ...
	 * </p>
	 * <p>
	 * This should be one of the identifiers used by <code>java.util.TimeZone</code> to refer to time zones, for example, <code>America/Chicago</code> or <code>Asia/Rangoon</code>.
	 * </p>
	 * 
	 * @param serverTimeZoneId
	...
	/**
	 * Looks up the supplied language code in the internally maintained table of language codes. Returns a DateFormatSymbols object configured with short month names corresponding to the code. If there is no corresponding entry in the table, the object returned will be that for <code>Locale.US</code>
	 * 
	 * @param languageCode
	 *            See {@link #setServerLanguageCode(String) serverLanguageCode}
	 * @return a DateFormatSymbols object configured with short month names corresponding to the supplied code, or with month names for <code>Locale.US</code> if there is no corresponding entry in the internal table.
	 */
	 ...
	 * 
	 * @param shortmonths
	 *            This should be as described in {@link #setShortMonthNames(String) shortMonthNames}
