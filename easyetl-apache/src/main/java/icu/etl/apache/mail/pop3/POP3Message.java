...
import icu.etl.apache.mail.internet.SharedInputStream;
...
public class POP3Message extends MimeMessage {
	...
	/*
	 * Our locking strategy is to always lock the POP3Folder before the POP3Message so we have to be careful to drop our lock before calling back to the folder to close it and notify of connection lost events.
	 */
	...
	public int getSize() throws MessagingException {
		...
		try {
			synchronized (this) {
				if (msgSize < 0) {
					/*
					 * Use LIST to determine the entire message size and subtract out the header size (which may involve loading the headers, which may load the content as a side effect). If the content is loaded as a side effect of loading the headers, get the size directly.
					 */
					if (headers == null)
						loadHeaders();
			...
			synchronized (this) {
				if (contentStream == null) {
					...
					if (headers == null || ((POP3Store) (folder.getStore())).forgetTopHeaders) {
						...
						/*
						 * Already have the headers, have to skip the headers in the content array and return the body.
						 *
						 * XXX - It seems that some mail servers return slightly different headers in the RETR results than were returned in the TOP results, so we can't depend on remembering the size of the headers from the TOP command and just skipping that many bytes. Instead, we have to process the content, skipping over the header until we come to the empty line that separates the header from the body.
						 */
//		    int offset = 0;
						for (;;) {
	...
	/**
	 * Invalidate the cache of content for this message object, causing it to be fetched again from the server the next time it is needed. If <code>invalidateHeaders</code> is true, invalidate the headers as well.
	 *
	...
	/**
	 * Fetch the header of the message and the first <code>n</code> lines of the raw content of the message. The headers and data are available in the returned InputStream.
	 *
	 * @param n
	 ...
	 *                because the underlying implementation does not support modification
