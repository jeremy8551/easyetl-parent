...
/**
 * This class represents an Internet email address using the syntax of <a href="http://www.ietf.org/rfc/rfc822.txt">RFC822</a>. Typical address syntax is of the form "user@host.domain" or "Personal Name <user@host.domain>".
 *
 * @author Bill Shannon
 * @author John Mani
 */
...
public class InternetAddress extends Address implements Cloneable {
	 ...
	 * <p>
	 *
	 * This field and the <code>personal</code> field track each other, so if a subclass sets one of these fields directly, it should set the other to <code>null</code>, so that it is suitably recomputed.
	 */
	 ...
	 * <p>
	 *
	 * Parse the given string and create an InternetAddress. See the <code>parse</code> method for details of the parsing. The address is parsed using "strict" parsing. This constructor does <b>not</b> perform the additional syntax checks that the <code>InternetAddress(String address, boolean strict)</code> constructor does when <code>strict</code> is <code>true</code>. This constructor is equivalent to <code>InternetAddress(address, false)</code>.
	 *
	 * @param address
	...
	public InternetAddress(String address) throws AddressException {
		...
		/*
		 * Now copy the contents of the single address we parsed into the current object, which will be returned from the constructor. XXX - this sure is a round-about way of getting this done.
		 */
	...
	/**
	 * Parse the given string and create an InternetAddress. If <code>strict</code> is false, the detailed syntax of the address isn't checked.
	 *
	 * @param address
	 *            the address in RFC822 format
	 * @param strict
	...
	/*
	 * Return the length of the first segment within this string. If no segments exist, the length of the whole line is returned.
	 */
	...
	/*
	 * Return the length of the last segment within this string. If no segments exist, the length of the whole line plus <code>used</code> is returned.
	 */
	...
	/**
	 * Parse the given sequence of addresses into InternetAddress objects. If <code>strict</code> is false, the full syntax rules for individual addresses are not enforced. If <code>strict</code> is true, many (but not all) of the RFC822 syntax rules are enforced.
	 * <p>
	 *
	 * To better support the range of "invalid" addresses seen in real messages, this method enforces fewer syntax rules than the <code>parse</code> method when the strict flag is false and enforces more rules when the strict flag is true. If the strict flag is false and the parse is successful in separating out an email address or addresses, the syntax of the addresses themselves is not checked.
	 *
	 * @param addresslist
	 *            comma separated address strings
	 * @param strict
	 ...
	 *
	 * XXX - This is complex enough that it ought to be a real parser, not this ad-hoc mess, and because of that, this is not perfect.
	 *
	...
	private static InternetAddress[] parse(String s, boolean strict, boolean parseHdr) throws AddressException {
		...
		if (start >= 0) {
			/*
			 * The last token, add this to our InternetAddress vector. Note that this block of code should be identical to the block above for "case ','".
			 */
			if (end == -1)
				end = index;
			String addr = s.substring(start, end).trim();
	...
	/**
	 * Check that the address is a valid "mailbox" per RFC822. (We also allow simple names.)
	 *
	 * XXX - much more to check XXX - doesn't handle domain-literals properly (but no one uses them)
	 */
	private static void checkAddress(String addr, boolean routeAddr, boolean validate) throws AddressException {
		...
		/*
		 * The rest should be "local@domain", but we allow simply "local" unless called from validate.
		 */
		String local;
		String domain;
		if ((i = addr.indexOf('@', start)) >= 0) {
			...
			/*
			 * No '@' so it's not *really* an RFC822 address, but still we allow some simple forms.
			 */
			local = addr;
			domain = null;
		}
	...
	/**
	 * Return the members of a group address. A group may have zero, one, or more members. If this address is not a group, null is returned. The <code>strict</code> parameter controls whether the group list is parsed using strict RFC 822 rules or not. The parsing is done using the <code>parseHeader</code> method.
	 *
