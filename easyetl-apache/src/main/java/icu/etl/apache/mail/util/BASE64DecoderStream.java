...
public class BASE64DecoderStream extends FilterInputStream {
	...
	/**
	 * Create a BASE64 decoder that decodes the specified input stream. The System property <code>mail.mime.base64.ignoreerrors</code> controls whether errors in the encoded data cause an exception or are ignored. The default is false (errors cause exception).
	 *
	 * @param in
	 *            the input stream
	 */
	...
	private int decode(byte[] outbuf, int pos, int len) throws IOException {
		...
		while (len >= 3) {
			/*
			 * We need 4 valid base64 characters before we start decoding. We skip anything that's not a valid base64 character (usually just CRLF).
			 */
			int got = 0;
			int val = 0;
			while (got < 4) {
	...
	/**
	 * Base64 decode a byte array. No line breaks are allowed. This method is suitable for short strings, such as those in the IMAP AUTHENTICATE protocol, but not to decode the entire content of a MIME part.
	 *
	...
	/***
	 * begin TEST program *** public static void main(String argv[]) throws Exception { FileInputStream infile = new FileInputStream(argv[0]); BASE64DecoderStream decoder = new BASE64DecoderStream(infile); int c;
	 * 
