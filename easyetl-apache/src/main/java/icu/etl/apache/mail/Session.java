...
import icu.etl.apache.mail.imap.IMAPSSLStore;
import icu.etl.apache.mail.imap.IMAPStore;
import icu.etl.apache.mail.pop3.POP3SSLStore;
import icu.etl.apache.mail.pop3.POP3Store;
import icu.etl.apache.mail.smtp.SMTPSSLTransport;
import icu.etl.apache.mail.smtp.SMTPTransport;
 ...
 * <p>
 *
 * The Session class provides access to the protocol providers that implement the <code>Store</code>, <code>Transport</code>, and related classes. The protocol providers are configured using the following files:
 * <ul>
 * <li><code>javamail.providers</code> and <code>javamail.default.providers</code></li>
 * <li><code>javamail.address.map</code> and <code>javamail.default.address.map</code></li>
 * </ul>
 * <p>
 * Each <code>javamail.</code><i>X</i> resource file is searched for using three methods in the following order:
 * <ol>
 ...
 * </ol>
 * <p>
 * The first method allows the user to include their own version of the resource file by placing it in the <code>lib</code> directory where the <code>java.home</code> property points. The second method allows an application that uses the JavaMail APIs to include their own resource files in their application's or jar file's <code>META-INF</code> directory. The <code>javamail.default.</code><i>X</i> default files are part of the JavaMail <code>mail.jar</code> file.
 * <p>
 *
 ...
 * <p>
 *
 * <b><code>javamail.providers</code></b> and <b><code>javamail.default.providers</code></b>
 * <p>
 *
 ...
 * <p>
 *
 * Here's an example of <code>META-INF/javamail.default.providers</code> file contents:
 * 
 * <pre>
 ...
 * <p>
 *
 * <b><code>javamail.address.map</code></b> and <b><code>javamail.default.address.map</code></b>
 * <p>
 *
...
public final class Session {
	 ...
	 *            It is expected that the client supplies values for the properties listed in Appendix A of the JavaMail spec (particularly mail.store.protocol, mail.transport.protocol, mail.host, mail.user, and mail.from) as the defaults are unlikely to work in all cases.
	 * @param authenticator
	 *            Authenticator object. Used only if a new Session object is created. Otherwise, it must match the Authenticator used to create the Session.
	...
	public static synchronized Session getDefaultInstance(Properties props, Authenticator authenticator) {
		if (defaultSession == null)
			...
			else if (defaultSession.authenticator != null && authenticator != null && defaultSession.authenticator.getClass().getClassLoader() == authenticator.getClass().getClassLoader())
	...
	/**
	 * Set the stream to be used for debugging output for this session. If <code>out</code> is null, <code>System.out</code> will be used. Note that debugging output that occurs before any session is created, as a result of setting the <code>mail.debug</code> system property, will always be sent to <code>System.out</code>.
	 *
	 * @param out
	...
	/**
	 * Returns the stream to be used for debugging output. If no stream has been set, <code>System.out</code> is returned.
	 *
	 ...
	 *
	 * @param provider
	 *            Currently configured Provider which will be set as the default for the protocol
	 * @exception NoSuchProviderException
	...
	/**
	 * Get a Store object that implements this user's desired Store protocol. The <code>mail.store.protocol</code> property specifies the desired protocol. If an appropriate Store object is not obtained, NoSuchProviderException is thrown
	 *
	...
	/**
	 * Get a Transport object that implements this user's desired Transport protcol. The <code>mail.transport.protocol</code> property specifies the desired protocol. If an appropriate Transport object cannot be obtained, MessagingException is thrown.
	 *
	 ...
	 *
	 * The "scheme" part of the URL string (Refer RFC 1738) is used to locate the Transport protocol.
	 * <p>
	 *
	 * @param url
	 ...
	 * @exception NoSuchProviderException
	 *                thrown when the class cannot be found or when it does not have the correct constructor (Session, URLName), or if it is not derived from Service.
	 */
	 ...
	 * @return the authentication which was collected by the authenticator; may be null.
	 */
	...
	private void loadProviders(Class cl) {
		...
		if (providers.size() == 0) {
			...
			addProvider(new Provider(Provider.Type.STORE, "imap", IMAPStore.class.getName(), "Sun Microsystems, Inc.", version));
			addProvider(new Provider(Provider.Type.STORE, "imaps", IMAPSSLStore.class.getName(), "Sun Microsystems, Inc.", version));
			addProvider(new Provider(Provider.Type.STORE, "pop3", POP3Store.class.getName(), "Sun Microsystems, Inc.", version));
			addProvider(new Provider(Provider.Type.STORE, "pop3s", POP3SSLStore.class.getName(), "Sun Microsystems, Inc.", version));
			addProvider(new Provider(Provider.Type.TRANSPORT, "smtp", SMTPTransport.class.getName(), "Sun Microsystems, Inc.", version));
			addProvider(new Provider(Provider.Type.TRANSPORT, "smtps", SMTPSSLTransport.class.getName(), "Sun Microsystems, Inc.", version));
		}
	...
	/**
	 * Set the default transport protocol to use for addresses of the specified type. Normally the default is set by the <code>javamail.default.address.map</code> or <code>javamail.address.map</code> files or resources.
	 *
	 * @param addresstype
	 *            type of address
	 * @param protocol
