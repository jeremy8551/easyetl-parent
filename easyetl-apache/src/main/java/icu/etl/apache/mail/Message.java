 ...
 * <p>
 *
 * Message defines some new attributes in addition to those defined in the <code>Part</code> interface. These attributes specify meta-data for the message - i.e., addressing and descriptive information about the message.
 * <p>
 *
 ...
 * 
 * To send a message, an appropriate subclass of Message (e.g., MimeMessage) is instantiated, the attributes and content are filled in, and the message is sent using the <code>Transport.send</code> method.
 * <p>
 *
 * @author John Mani
 * @author Bill Shannon
...
public abstract class Message implements Part {
	 ...
	 * <p>
	 *
	 * This method returns <code>null</code> if this attribute is not present in this message. Returns an empty array if this attribute is present, but contains no addresses.
	 *
	...
	/**
	 * This inner class defines the types of recipients allowed by the Message class. The currently defined types are TO, CC and BCC.
	 *
	 ...
	 *
	 * The following code sample shows how to use this class to obtain the "TO" recipients from a message. <blockquote>
	 * 
	 * <pre>
	 *
	...
	public static class RecipientType implements Serializable {
		...
		/**
		 * When deserializing a RecipientType, we need to make sure to return only one of the known static final instances defined in this class. Subclasses must implement their own <code>readResolve</code> method that checks for their known instances before calling this super method.
		 */
	 ...
	 * <p>
	 *
	 * This method returns <code>null</code> if no recipients of the given type are present in this message. It may return an empty array if the header is present, but contains no addresses.
	 *
	 * @param type
	 *            the recipient type
	 * @return array of Address objects
	...
	/**
	 * Get all the recipient addresses for the message. The default implementation extracts the TO, CC, and BCC recipients using the <code>getRecipients</code> method.
	 * <p>
	 *
	 * This method returns <code>null</code> if none of the recipient headers are present in this message. It may Return an empty array if any recipient header is present, but contains no addresses.
	 *
	 * @return array of Address objects
	 * @exception MessagingException
	 * @see Message.RecipientType#TO
	 ...
	 * <p>
	 *
	 * This method returns <code>null</code> if the corresponding header is not present. Returns an empty array if the header is present, but contains no addresses.
	 *
	 ...
	 * <p>
	 *
	 * Modifying any of the flags in this returned Flags object will not affect the flags of this message. Use <code>setFlags()</code> to do that.
	 * <p>
	 *
	...
	/**
	 * Set the specified flags on this message to the specified value. Note that any flags in this message that are not specified in the given <code>Flags</code> object are unaffected.
	 * <p>
	 *
	 * This will result in a <code>MessageChangedEvent</code> being delivered to any MessageChangedListener registered on this Message's containing folder.
	 *
	 * @param flag
	...
	/**
	 * Checks whether this message is expunged. All other methods except <code>getMessageNumber()</code> are invalid on an expunged Message object.
	 * <p>
	 *
	 * Messages that are expunged due to an explict <code>expunge()</code> request on the containing Folder are removed from the Folder immediately. Messages that are externally expunged by another source are marked "expunged" and return true for the isExpunged() method, but they are not removed from the Folder until an explicit <code>expunge()</code> is done on the Folder.
	 * <p>
	 * 
	 * See the description of <code>expunge()</code> for more details on expunge handling.
	 *
	 * @see Folder#expunge
	 */
	...
	/**
	 * Get a new Message suitable for a reply to this message. The new Message will have its attributes and headers set up appropriately. Note that this new message object will be empty, that is, it will <strong>not</strong> have a "content". These will have to be suitably filled in by the client.
	 * <p>
	 *
