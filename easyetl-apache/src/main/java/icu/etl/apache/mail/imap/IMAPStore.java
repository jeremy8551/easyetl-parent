...
import icu.etl.apache.mail.imap.protocol.Namespaces;
import icu.etl.apache.mail.AuthenticationFailedException;
import icu.etl.apache.mail.Folder;
import icu.etl.apache.mail.MessagingException;
import icu.etl.apache.mail.PasswordAuthentication;
import icu.etl.apache.mail.Quota;
import icu.etl.apache.mail.QuotaAwareStore;
import icu.etl.apache.mail.Session;
import icu.etl.apache.mail.Store;
import icu.etl.apache.mail.StoreClosedException;
import icu.etl.apache.mail.URLName;
 ...
 * <p>
 *
 * Applications that need to make use of IMAP-specific features may cast a <code>Store</code> object to an <code>IMAPStore</code> object and use the methods on this class. The {@link #getQuota getQuota} and {@link #setQuota setQuota} methods support the IMAP QUOTA extension. Refer to <A HREF="http://www.ietf.org/rfc/rfc2087.txt">RFC 2087</A> for more information.
 * <p>
 *
 ...
 */
/*
 * This package is implemented over the "imap.protocol" package, which implements the protocol-level commands. <p>
 *
 * A connected IMAPStore maintains a pool of IMAP protocol objects for use in communicating with the IMAP server. The IMAPStore will create the initial AUTHENTICATED connection and seed the pool with this connection. As folders are opened and new IMAP protocol objects are needed, the IMAPStore will provide them from the connection pool, or create them if none are available. When a folder is closed, its IMAP protocol object is returned to the connection pool if the pool is not over capacity. The pool size can be configured by setting the mail.imap.connectionpoolsize property. <p>
 *
 * A mechanism is provided for timing out idle connection pool IMAP protocol objects. Timed out connections are closed and removed (pruned) from the connection pool. The time out interval can be configured via the mail.imap.connectionpooltimeout property. <p>
 *
 * The connected IMAPStore object may or may not maintain a separate IMAP protocol object that provides the store a dedicated connection to the IMAP server. This is provided mainly for compatibility with previous implementations of JavaMail and is determined by the value of the mail.imap.separatestoreconnection property. <p>
 *
 * An IMAPStore object provides closed IMAPFolder objects thru its list() and listSubscribed() methods. A closed IMAPFolder object acquires an IMAP protocol object from the store to communicate with the server. When the folder is opened, it gets its own protocol object and thus its own, separate connection to the server. The store maintains references to all 'open' folders. When a folder is/gets closed, the store removes it from its list. When the store is/gets closed, it closes all open folders in its list, thus cleaning up all open connections to the server. <p>
 *
 * A mutex is used to control access to the connection pool resources. Any time any of these resources need to be accessed, the following convention should be followed:
 *
 * synchronized (pool) { // ACQUIRE LOCK // access connection pool resources } // RELEASE LOCK <p>
 *
 * The locking relationship between the store and folders is that the store lock must be acquired before a folder lock. This is currently only applicable in the store's cleanup method. It's important that the connection pool lock is not not held when calling into folder objects. The locking hierarchy is that a folder lock must be acquired before any connection pool operations are performed. <p>
 *
 * The IMAPStore implements the ResponseHandler interface and listens to BYE or untagged OK-notification events from the server. <p>
 */
...
public class IMAPStore extends Store implements QuotaAwareStore, ResponseHandler {
	...
	protected IMAPStore(Session session, URLName url, String name, int defaultPort, boolean isSSL) {
		...
		if (s != null) {
			...
			if (debug)
				out.println("DEBUG: mail.imap.statuscachetimeout: " + statusCacheTimeout);
		}
		s = session.getProperty("mail." + name + ".appendbuffersize");
		if (s != null) {
			appendBufferSize = Integer.parseInt(s);
			if (debug)
				out.println("DEBUG: mail.imap.appendbuffersize: " + appendBufferSize);
		}
		...
		if (s != null) {
			...
			if (pool.debug)
				out.println("DEBUG: mail.imap.connectionpoolsize: " + pool.poolSize);
		}
		...
		if (s != null) {
			...
			if (pool.debug)
				out.println("DEBUG: mail.imap.connectionpooltimeout: " + pool.clientTimeoutInterval);
		}
		...
		if (s != null) {
			...
			if (pool.debug)
				out.println("DEBUG: mail.imap.servertimeout: " + pool.serverTimeoutInterval);
		}
		...
		if (s != null) {
			proxyAuthUser = s;
			if (debug)
				out.println("DEBUG: mail.imap.proxyauth.user: " + proxyAuthUser);
		}
		...
		if (s != null) {
			authorizationID = s;
			if (debug)
				out.println("DEBUG: mail.imap.sasl.authorizationid: " + authorizationID);
		}
	 ...
	 * <p>
	 *
	 * The <code>host</code>, <code>user</code>, and <code>password</code> parameters must all be non-null. If the authentication mechanism being used does not require a password, an empty string or other suitable dummy password should be used.
	 */
	protected synchronized boolean protocolConnect(String host, int pport, String user, String password) throws MessagingException {
		...
		try {
			...
			if (poolEmpty) {
				...
				if (debug)
					out.println("DEBUG: protocolConnect login" + ", host=" + host + ", user=" + user + ", password=" + (password != null ? "<non-null>" : "<null>"));
	...
	private void login(IMAPProtocol p, String u, String pw) throws ProtocolException {
		...
		else if ((p.hasCapability("AUTH-LOGIN") || p.hasCapability("AUTH=LOGIN")) && !disableAuthLogin)
			p.authlogin(u, pw);
	...
	/*
	 * Get a new authenticated protocol object for this Folder. Also store a reference to this folder in our list of open folders.
	 */
	IMAPProtocol getProtocol(IMAPFolder folder) throws MessagingException {
		...
		while (p == null) {
			...
			synchronized (pool) {
				...
				// create a new one.
				if (pool.authenticatedConnections.isEmpty() || (pool.separateStoreConnection && pool.authenticatedConnections.size() == 1) || (pool.borrowedStoreConnections > 0 && pool.authenticatedConnections.size() == 1)) {
					
					if (debug)
						out.println("DEBUG: no connections in the pool, " + "creating a new one");
					try {
						/*
						 * Some authentication systems use one time passwords or tokens, so each authentication request requires a new password. This "kludge" allows a callback to application code to get a new password.
						 *
						...
						p = new IMAPProtocol(name, host, port, session.getDebug(), session.getDebugOut(), session.getProperties(), isSSL);
					...
					if (debug)
						out.println("DEBUG: connection available -- size: " + pool.authenticatedConnections.size());
	 ...
	 *
	 * IMAPProtocol p = null; try { p = getStoreProtocol(); // perform the command } catch (ConnectionException cex) { throw new StoreClosedException(this, cex.getMessage()); } catch (WhateverException ex) { // handle it } finally { releaseStoreProtocol(p); if (p == null) { // failed to get a Store connection // have to force Store to be closed cleanup(); } }
	 */
	IMAPProtocol getStoreProtocol() throws ProtocolException {
		...
		synchronized (pool) {
			...
			if (pool.authenticatedConnections.isEmpty()) {
				...
				if (pool.debug)
					out.println("DEBUG: getStoreProtocol() - " + "connection available -- size: " + pool.authenticatedConnections.size());
			...
			if (!pool.separateStoreConnection) {
				...
				if (pool.debug)
					out.println("DEBUG: getStoreProtocol() -- " + "borrowedStoreConnections: " + pool.borrowedStoreConnections);
			}
	...
	boolean allowReadOnlySelect() {
		String s = session.getProperty("mail." + name + ".allowreadonlyselect");
	...
	boolean isConnectionPoolFull() {
		...
		synchronized (pool) {
			if (pool.debug)
				out.println("DEBUG: current size: " + pool.authenticatedConnections.size() + "   pool size: " + pool.poolSize);
		...
		synchronized (pool) {
			if (protocol != null) {
				...
				if (!isConnectionPoolFull()) {
					...
					if (debug)
						out.println("DEBUG: added an " + "Authenticated connection -- size: " + pool.authenticatedConnections.size());
				} else {
					if (debug)
	...
	void releaseStoreProtocol(IMAPProtocol protocol) {
		...
		synchronized (pool) {
			...
			if (!pool.separateStoreConnection) {
				...
				if (pool.debug)
					out.println("DEBUG: releaseStoreProtocol() -- " + "borrowedStoreConnections: " + pool.borrowedStoreConnections);
			}
	...
	private void timeoutConnections() {
		...
		synchronized (pool) {
			...
			if (System.currentTimeMillis() - pool.lastTimePruned > pool.pruningInterval && pool.authenticatedConnections.size() > 1) {
				...
				if (pool.debug) {
					out.println("DEBUG: checking for connections " + "to prune: " + (System.currentTimeMillis() - pool.lastTimePruned));
					out.println("DEBUG: clientTimeoutInterval: " + pool.clientTimeoutInterval);
				}
				...
				for (int index = pool.authenticatedConnections.size() - 1; index > 0; index--) {
					p = (IMAPProtocol) pool.authenticatedConnections.elementAt(index);
					if (pool.debug) {
						out.println("DEBUG: protocol last used: " + (System.currentTimeMillis() - p.getTimestamp()));
					}
	...
	public synchronized boolean isConnected() {
		 ...
		 *
		 * (2) fail because the server returns NO or BAD, in which case we ignore it since we can't really do anything. (2) fail because a BYE response is obtained from the server (3) fail because the socket.write() to the server fails, in which case the iap.protocol() code converts the IOException into a BYE response.
		 *
		 * Thus, our BYE handler will take care of closing the Store in case our connection is really gone.
		 */
	...
	public void close() throws MessagingException {
		...
		try {
			...
			/*
			 * Have to drop the lock before calling cleanup. Yes, there's a potential race here. The pool could become empty after we check, in which case we'll just waste time getting a new connection and closing it. Or, the pool could be empty now and not empty by the time we get into cleanup, but that's ok because cleanup will just close the connection.
			 */
			if (isEmpty) {
				if (pool.debug)
			...
			/*
			 * We have to remove the protocol from the pool so that, when our response handler processes the BYE response and calls cleanup, which calls emptyConnection, that we don't try to log out this connection twice.
			 */
			synchronized (pool) {
			 ...
			 * LOGOUT.
			 *
			 * Note that protocol.logout() closes the server socket connection, regardless of what happens ..
			 *
			 * Also note that protocol.logout() results in a BYE response (As per rfc 2060, BYE is a *required* response to LOGOUT). In fact, even if protocol.logout() fails with an IOException (if the server connection is dead), iap.Protocol.command() converts that exception into a BYE response. So, I depend on my BYE handler to do the Store cleanup.
			 */
			protocol.logout();
		} catch (ProtocolException pex) {
	...
	/**
	 * Cleanup before dying. If force is true, we force the folders to close abruptly without waiting for the server. Used when the store connection times out.
	 *
	 */
	...
	public Folder getFolder(URLName url) throws MessagingException {
		checkConnected();
		return new IMAPFolder(url.getFile(), IMAPFolder.UNKNOWN_SEPARATOR, this);
	}
	...
	/**
	 * Set the quotas for the quota root specified in the quota argument. Typically this will be one of the quota roots obtained from the <code>getQuota</code> method, but it need not be.
	 *
	 * @param quota
	 *            the quota to set
	 * @exception MessagingException
