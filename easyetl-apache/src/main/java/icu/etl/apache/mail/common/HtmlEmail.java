...
import icu.etl.apache.mail.activation.URLDataSource;
import icu.etl.apache.mail.internet.MimeBodyPart;
 ...
 *
 * <p>
 * To send an email in HTML, one should create a <code>HtmlEmail</code>, then use the {@link #setFrom(String)}, {@link #addTo(String)} etc. methods. The HTML content can be set with the {@link #setHtmlMsg(String)} method. The alternative text content can be set with {@link #setTextMsg(String)}.
 *
 * <p>
 * Either the text or HTML can be omitted, in which case the "main" part of the multipart becomes whichever is supplied rather than a <code>multipart/alternative</code>.
 *
 * <h3>Embedding Images and Media</h3>
 *
 * <p>
 * It is also possible to embed URLs, files, or arbitrary <code>DataSource</code>s directly into the body of the mail:
 * 
 * <pre>
 * <code>
 ...
 *
 * <p>
 * Embedded entities are tracked by their name, which for <code>File</code>s is the filename itself and for <code>URL</code>s is the canonical path. It is an error to bind the same name to more than one entity, and this class will attempt to validate that for <code>File</code>s and <code>URL</code>s. When embedding a <code>DataSource</code>, the code uses the <code>equals()</code> method defined on the <code>DataSource</code>s to make the determination.
 *
 * @since 1.0
 * @author <a href="mailto:unknown">Regis Koenig</a>
 * @author <a href="mailto:sean@informage.net">Sean Legassick</a>
 * @version $Id: HtmlEmail.java 578501 2007-09-22 21:18:49Z bspeakmon $
 */
...
public class HtmlEmail extends MultiPartEmail {
	 ...
	 * @throws EmailException
	 *             if msg is null or empty; see javax.mail.internet.MimeBodyPart for definitions
	 * @since 1.0
	 */
	 ...
	 * @throws EmailException
	 *             when URL supplied is invalid or if <code> is null or empty; also see {@link MimeBodyPart} for definitions
	 * 			
	 ...
	 *
	 * <p>
	 * While functionally the same as passing <code>URLDataSource</code> to {@link #embed(DataSource, String, String)}, this method attempts to validate the URL before embedding it in the message and will throw <code>EmailException</code> if the validation fails. In this case, the <code>HtmlEmail</code> object will not be changed.
	 *
	 * <p>
	 * NOTE: Clients should take care to ensure that different URLs are bound to different names. This implementation tries to detect this and throw <code>EmailException</code>. However, it is not guaranteed to catch all cases, especially when the URL refers to a remote HTTP host that may be part of a virtual host cluster.
	 *
	 * @param url
	 *            The URL of the file.
	 * @param name
	 ...
	 * @return A String with the Content-ID of the file.
	 * @throws EmailException
	 *             when the supplied <code>File</code> cannot be used; also see {@link MimeBodyPart} for definitions
	 * 			
	 ...
	 *
	 * <p>
	 * While functionally the same as passing <code>FileDataSource</code> to {@link #embed(DataSource, String, String)}, this method attempts to validate the file before embedding it in the message and will throw <code>EmailException</code> if the validation fails. In this case, the <code>HtmlEmail</code> object will not be changed.
	 *
	 * @param file
	 ...
	 * @return A String with the Content-ID of the file.
	 * @throws EmailException
	 *             when the supplied <code>File</code> cannot be used or if the file has already been embedded; also see {@link MimeBodyPart} for definitions
	 * @since 1.1
	 */
	...
	/**
	 * Embeds the specified <code>DataSource</code> in the HTML using a randomly generated Content-ID. Returns the generated Content-ID string.
	 *
	 * @param dataSource
	...
	/**
	 * Embeds the specified <code>DataSource</code> in the HTML using the specified Content-ID. Returns the specified Content-ID string.
	 *
	 * @param dataSource
	 *            the <code>DataSource</code> to embed
	 * @param name
	...
	private void build() throws MessagingException, EmailException {
		...
		if (EmailUtils.isNotEmpty(this.text)) {
			...
			if (EmailUtils.isNotEmpty(this.charset)) {
				msgText.setContent(this.text, TEXT_PLAIN + "; charset=" + this.charset);
			} else {
				msgText.setContent(this.text, TEXT_PLAIN);
			}
		}
		...
		if (EmailUtils.isNotEmpty(this.html)) {
			...
			// apply default charset if one has been set
			if (EmailUtils.isNotEmpty(this.charset)) {
				msgHtml.setContent(this.html, TEXT_HTML + "; charset=" + this.charset);
			} else {
				msgHtml.setContent(this.html, TEXT_HTML);
			}
	...
	private static class InlineImage {
		...
		/**
		 * Creates an InlineImage object to represent the specified content ID and <code>MimeBodyPart</code>.
		 * 
		 * @param cid
		...
		/**
		 * Returns the <code>MimeBodyPart</code> that contains the encoded InlineImage data.
		 * 
		 * @return the <code>MimeBodyPart</code> containing the encoded InlineImage data
		 */
