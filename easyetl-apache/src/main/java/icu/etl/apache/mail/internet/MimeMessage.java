...
package icu.etl.apache.mail.internet;
...
import icu.etl.apache.mail.activation.DataHandler;
import icu.etl.apache.mail.search.MessageIDTerm;
import icu.etl.apache.mail.Address;
import icu.etl.apache.mail.Flags;
import icu.etl.apache.mail.Folder;
import icu.etl.apache.mail.IllegalWriteException;
import icu.etl.apache.mail.Message;
import icu.etl.apache.mail.MessagingException;
import icu.etl.apache.mail.Multipart;
import icu.etl.apache.mail.Session;
import icu.etl.apache.mail.util.ASCIIUtility;
import icu.etl.apache.mail.util.LineOutputStream;
import icu.etl.apache.mail.util.SharedByteArrayInputStream;
import icu.etl.apache.mail.Part;

/**
 * This class represents a MIME style email message. It implements the <code>Message</code> abstract class and the <code>MimePart</code> interface.
 * <p>
 * <p>
...
public class MimeMessage extends Message implements MimePart {
    ...
    /**
     * If the data for this message was supplied by an InputStream that implements the SharedInputStream interface, <code>contentStream</code> is another such stream representing the content of this message. In this case, <code>content</code> will be null.
     *
     * @since JavaMail 1.2
     */
    ...
    /**
     * Does the <code>saveChanges</code> method need to be called on this message? This flag is set to false by the public constructor and set to true by the <code>saveChanges</code> method. The <code>writeTo</code> method checks this flag and calls the <code>saveChanges</code> method as necessary. This avoids the common mistake of forgetting to call the <code>saveChanges</code> method on a newly constructed message.
     *
     * @since JavaMail 1.2
     */
    ...
    /**
     * Default constructor. An empty message object is created. The <code>headers</code> field is set to an empty InternetHeaders object. The <code>flags</code> field is set to an empty Flags object. The <code>modified</code> flag is set to true.
     */
    ...
    /**
     * Constructs a new MimeMessage with content initialized from the <code>source</code> MimeMessage. The new message is independent of the original.
     * <p>
     * <p>
    ...
    /**
     * Parse the InputStream setting the <code>headers</code> and <code>content</code> fields appropriately. Also resets the <code>modified</code> flag.
     * <p>
     * <p>
    ...
    /**
     * Returns the value of the RFC 822 "From" header fields. If this header field is absent, the "Sender" header field is used. If the "Sender" header field is also absent, <code>null</code> is returned.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
    ...
    /**
     * Set the RFC 822 "From" header field. Any existing values are replaced with the given address. If address is <code>null</code>, this header is removed.
     *
    ...
    /**
     * Set the RFC 822 "From" header field using the value of the <code>InternetAddress.getLocalAddress</code> method.
     *
     * @throws IllegalWriteException if the underlying implementation does not support modification of existing values
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException
    ...
    /**
     * Returns the value of the RFC 822 "Sender" header field. If the "Sender" header field is absent, <code>null</code> is returned.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
    ...
    /**
     * Set the RFC 822 "Sender" header field. Any existing values are replaced with the given address. If address is <code>null</code>, this header is removed.
     *
     * @param address the sender of this message
     * @throws IllegalWriteException if the underlying implementation does not support modification of existing values
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
	 ...
	 * @throws MessagingException if header could not be retrieved
    ...
    /**
     * Set the specified recipient type to the given addresses. If the address parameter is <code>null</code>, the corresponding recipient field is removed.
     *
    ...
    /**
     * Return the value of the RFC 822 "Reply-To" header field. If this header is unavailable or its value is absent, then the <code>getFrom</code> method is called and its value is returned.
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
     * @throws MessagingException
    ...
    /**
     * Set the RFC 822 "Reply-To" header field. If the address parameter is <code>null</code>, this header is removed.
     *
     * @throws IllegalWriteException if the underlying implementation does not support modification of existing values
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException
     ...
     * @throws IllegalWriteException if the underlying implementation does not support modification of existing values
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException    An UnsupportedEncodingException may be included in the exception chain if the charset conversion fails.
     */
    ...
    /**
     * Set the RFC 822 "Date" header field. This is the date on which the creator of the message indicates that the message is complete and ready for delivery. If the date parameter is <code>null</code>, the existing "Date" field is removed.
     *
    ...
    /**
     * Returns the Date on this message was received. Returns <code>null</code> if this date cannot be obtained.
     * <p>
     * <p>
     ...
     * <p>
     * <p>
     * This implementation returns the size of the <code>content</code> array (if not null), or, if <code>contentStream</code> is not null, and the <code>available</code> method returns a positive number, it returns that number as the size. Otherwise, it returns -1.
     *
    ...
    /**
     * Is this Part of the specified MIME type? This method compares <strong>only the <code>primaryType</code> and <code>subType</code></strong>. The parameters of the content types are ignored.
     * <p>
     * <p>
     * For example, this method will return <code>true</code> when comparing a Part of content type <strong>"text/plain"</strong> with <strong>"text/plain; charset=foobar"</strong>.
     * <p>
     * <p>
     * If the <code>subType</code> of <code>mimeType</code> is the special character '*', then the subtype is ignored during the comparison.
     */
     ...
     * <p>
     * <p>
     * If the Content-Disposition field is unavailable, <code>null</code> is returned.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
    ...
    /**
     * Set the "Content-Disposition" header field of this Message. If <code>disposition</code> is null, any existing "Content-Disposition" header field is removed.
     *
     * @throws IllegalWriteException if the underlying implementation does not support modification
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException
    ...
    /**
     * Returns the content transfer encoding from the "Content-Transfer-Encoding" header field. Returns <code>null</code> if the header is unavailable or its value is absent.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
    ...
    /**
     * Returns the value of the "Content-ID" header field. Returns <code>null</code> if the field is unavailable or its value is absent.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
    ...
    /**
     * Set the "Content-ID" header field of this Message. If the <code>cid</code> parameter is null, any existing "Content-ID" is removed.
     *
     * @throws IllegalWriteException if the underlying implementation does not support modification
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException
    ...
    /**
     * Return the value of the "Content-MD5" header field. Returns <code>null</code> if this field is unavailable or its value is absent.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
    ...
    /**
     * Set the "Content-Description" header field for this Message. If the description parameter is <code>null</code>, then any existing "Content-Description" fields are removed.
     * <p>
     * <p>
    ...
    /**
     * Get the languages specified in the "Content-Language" header field of this message. The Content-Language header is defined by RFC 1766. Returns <code>null</code> if this field is unavailable or its value is absent.
     * <p>
     * <p>
     * This implementation uses the <code>getHeader</code> method to obtain the requisite header field.
     *
     ...
     * <p>
     * <p>
     * The default implementation provided here uses the <code>getHeader</code> method to return the value of the "Message-ID" field.
     *
     * @return Message-ID
     * @throws MessagingException if the retrieval of this field causes any exception.
     * @see MessageIDTerm
     * @since JavaMail 1.1
     */
     ...
     * <p>
     * <p>
     * Returns the value of the "filename" parameter from the "Content-Disposition" header field of this message. If it's not available, returns the value of the "name" parameter from the "Content-Type" header field of this BodyPart. Returns <code>null</code> if both are absent.
     * <p>
     * <p>
     ...
     * <p>
     * <p>
     * This implementation obtains the input stream from the DataHandler, that is, it invokes <code>getDataHandler().getInputStream()</code>.
     *
     ...
     * <p>
     * <p>
     * This implementation returns a SharedInputStream, if <code>contentStream</code> is not null. Otherwise, it returns a ByteArrayInputStream constructed out of the <code>content</code> byte array.
     *
     * @see #content
     */
	 ...
	 * <p>
	 * <p>
	 * The implementation provided here works as follows. Note the use of the <code>getContentStream</code> method to generate the byte stream for the content. Also note that any transfer-decoding is done automatically within this method.
	 * <p>
	 *
	 * <blockquote>
	 *
     ...
     * <p>
     * <p>
     * Note that there may be a performance penalty if <code>text</code> is large, since this method may have to scan all the characters to determine what charset to use.
     * <p>
     * <p>
    ...
    /**
     * Get a new Message suitable for a reply to this message. The new Message will have its attributes and headers set up appropriately. Note that this new message object will be empty, i.e., it will <strong>not</strong> have a "content". These will have to be suitably filled in by the client.
     * <p>
     * <p>
     * If <code>replyToAll</code> is set, the new Message will be addressed to all recipients of this message. Otherwise, the reply will be addressed to only the sender of this message (using the value of the <code>getReplyTo</code> method).
     * <p>
     * <p>
     * The "Subject" field is filled in with the original subject prefixed with "Re:" (unless it already starts with "Re:"). The "In-Reply-To" header is set in the new message if this message has a "Message-Id" header. The <code>ANSWERED</code> flag is set in this message.
     *
    ...
    public Message reply(boolean replyToAll) throws MessagingException {
        ...
        /*
         * Have to manipulate the raw Subject header so that we don't lose any encoding information. This is safe because "Re:" isn't internationalized and (generally) isn't encoded. If the entire Subject header is encoded, prefixing it with "Re: " still leaves a valid and correct encoded header.
         */
     ...
     *
     * @throws IOException        if an error occurs writing to the stream or if an error is generated by the javax.activation layer.
     * @throws MessagingException
     ...
     * <p>
     * <p>
     * This implementation obtains the headers from the <code>headers</code> InternetHeaders object.
     *
    ...
    /**
     * Get all the headers for this header name, returned as a single String, with headers separated by the delimiter. If the delimiter is <code>null</code>, only the first header is returned.
     *
    ...
    /**
     * Return matching headers from this Message as an Enumeration of Header objects. This implementation obtains the headers from the <code>headers</code> InternetHeaders object.
     *
     * @throws MessagingException
     */
    ...
    /**
     * Return non-matching headers from this Message as an Enumeration of Header objects. This implementation obtains the header from the <code>headers</code> InternetHeaders object.
     *
     * @throws MessagingException
     */
    ...
    /**
     * Check whether the flag specified in the <code>flag</code> argument is set in this message.
     * <p>
     * <p>
     ...
     * <p>
     * <p>
     * If any part of a message's headers or contents are changed, <code>saveChanges</code> must be called to ensure that those changes are permanent. Otherwise, any such modifications may or may not be saved, depending on the folder implementation.
     * <p>
     * <p>
     ...
     * <p>
     * <p>
     * This method sets the <code>modified</code> flag to true, the <code>save</code> flag to true, and then calls the <code>updateHeaders<code> method.
     *
     * @throws IllegalWriteException if the underlying implementation does not support modification
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException
    ...
    /**
     * Update the Message-ID header. This method is called by the <code>updateHeaders</code> and allows a subclass to override only the algorithm for choosing a Message-ID.
     *
     * @since JavaMail 1.4
     */
    ...
    /**
     * Called by the <code>saveChanges</code> method to actually update the MIME headers. The implementation here sets the <code>Content-Transfer-Encoding</code> header (if needed and not already set), the <code>MIME-Version</code> header and the <code>Message-ID</code> header. Also, if the content of this message is a <code>MimeMultipart</code>, it's <code>updateHeaders</code> method is called.
     *
     * @throws IllegalWriteException if the underlying implementation does not support modification
     * @throws IllegalStateException if this message is obtained from a READ_ONLY folder.
     * @throws MessagingException
