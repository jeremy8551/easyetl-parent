...
import icu.etl.apache.mail.MessageAware;
import icu.etl.apache.mail.MessageContext;
import icu.etl.apache.mail.MessagingException;
import icu.etl.apache.mail.Multipart;
import icu.etl.apache.mail.MultipartDataSource;
 ...
 * <p>
 *
 * A MimeMultipart is obtained from a MimePart whose primary type is "multipart" (by invoking the part's <code>getContent()</code> method) or it can be created by a client as part of creating a new MimeMessage.
 * <p>
 *
 ...
 * <p>
 *
 * An application can directly construct a MIME multipart object of any subtype by using the <code>MimeMultipart(String subtype)</code> constructor. For example, to create a "multipart/alternative" object, use <code>new MimeMultipart("alternative")</code>.
 * <p>
 *
 * The <code>mail.mime.multipart.ignoremissingendboundary</code> property may be set to <code>false</code> to cause a <code>MessagingException</code> to be thrown if the multipart data does not end with the required end boundary line. If this property is set to <code>true</code> or not set, missing end boundaries are not considered an error and the final body part ends at the end of the data.
 * <p>
 *
 * The <code>mail.mime.multipart.ignoremissingboundaryparameter</code> System property may be set to <code>false</code> to cause a <code>MessagingException</code> to be thrown if the Content-Type of the MimeMultipart does not include a <code>boundary</code> parameter. If this property is set to <code>true</code> or not set, the multipart parsing code will look for a line that looks like a bounary line and use that as the boundary separating the parts.
 *
...
public class MimeMultipart extends Multipart {
	...
	/**
	 * Default constructor. An empty MimeMultipart object is created. Its content type is set to "multipart/mixed". A unique boundary string is generated and this string is setup as the "boundary" parameter for the <code>contentType</code> field.
	 * <p>
	 *
	...
	/**
	 * Construct a MimeMultipart object of the given subtype. A unique boundary string is generated and this string is setup as the "boundary" parameter for the <code>contentType</code> field.
	 * <p>
	 *
	 * MimeBodyParts may be added later.
	 */
	...
	/**
	 * Update headers. The default implementation here just calls the <code>updateHeaders</code> method on each of its children BodyParts.
	 * <p>
	 *
	...
	/**
	 * Parse the InputStream from our DataSource, constructing the appropriate MimeBodyParts. The <code>parsed</code> flag is set to true, and if true on entry nothing is done. This method is called by all other methods that need data for the body parts, to make sure the data has been parsed.
	 *
	 * @since JavaMail 1.2
	 */
	protected synchronized void parse() throws MessagingException {
		...
		try {
			...
			while ((line = lin.readLine()) != null) {
				/*
				 * Strip trailing whitespace. Can't use trim method because it's too aggressive. Some bogus MIME messages will include control characters in the boundary string.
				 */
				int i;
				...
				if (boundary != null) {
					...
					/*
					 * Boundary hasn't been defined, does this line look like a boundary? If so, assume it is the boundary and save it.
					 */
					if (line.startsWith("--")) {
						boundary = line;
						break;
			...
			/*
			 * Read and process body parts until we see the terminating boundary line (or EOF).
			 */
			boolean done = false;
			getparts: while (!done) {
				...
				for (;;) {
					if (bol) {
						/*
						 * At the beginning of a line, check whether the next line is a boundary.
						 */
						int i;
					...
					/*
					 * If we're at the end of the line, save the eol characters to be written out before the beginning of the next line.
					 */
					if (b == '\r' || b == '\n') {
						bol = true;
						if (sin != null)
	 ...
	 */
	/*
	 * Boyer-Moore version of parser. Keep both versions around until we're sure this new one works.
	 */
	private synchronized void parsebm() throws MessagingException {
		...
		try {
			...
			getparts: while (!done) {
				...
				/*
				 * These buffers contain the bytes we're checking for a match. inbuf is the current buffer and previnbuf is the previous buffer. We need the previous buffer to check that we're preceeded by an EOL.
				 */
				...
				for (;;) {
					...
					/*
					 * Get here if boundary didn't match, wasn't preceeded by EOL, or wasn't followed by whitespace or EOL.
					 */
