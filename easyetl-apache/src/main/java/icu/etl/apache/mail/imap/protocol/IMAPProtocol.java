...
package icu.etl.apache.mail.imap.protocol;
...
import icu.etl.apache.mail.internet.MimeUtility;
...
import icu.etl.apache.mail.iap.Argument;
...
import icu.etl.apache.mail.iap.ParsingException;
import icu.etl.apache.mail.iap.Protocol;
import icu.etl.apache.mail.iap.ProtocolException;
import icu.etl.apache.mail.iap.Response;
import icu.etl.apache.mail.imap.ACL;
import icu.etl.apache.mail.imap.AppendUID;
import icu.etl.apache.mail.imap.Rights;
...
public class IMAPProtocol extends Protocol {
		...
		while ((s = r.readAtom(']')) != null) {
			if (s.length() == 0) {
				if (r.peekByte() == (byte) ']')
					break;
				/*
				 * Probably found something here that's not an atom. Rather than loop forever or fail completely, we'll try to skip this bogus capability. This is known to happen with: Netscape Messaging Server 4.03 (built Apr 27 1999) that returns: * CAPABILITY * CAPABILITY IMAP4 IMAP4rev1 ... The "*" in the middle of the capability list causes us to loop forever here.
				 */
				r.skipToken();
			} else {
	...
	/**
	 * Returns <code>true</code> if the connection has been authenticated, either due to a successful login, or due to a PREAUTH greeting response.
	 */
	...
	/**
	 * Check whether the given capability is supported by this server. Returns <code>true</code> if so, otherwise returns false.
	 */
	...
	public void authlogin(String u, String p) throws ProtocolException {
		 ...
		 *
		 * Note that the encoded bytes should be sent "as-is" to the server, *not* as literals or quoted-strings.
		 *
		 ...
		 *
		 * Finally, format the line in a buffer so it can be sent as a single packet, to avoid triggering a bug in SUN's SIMS 2.0 server caused by patch 105346.
		 */
		...
		/*
		 * Dispatch untagged responses. NOTE: in our current upper level IMAP classes, we add the responseHandler to the Protocol object only *after* the connection has been authenticated. So, for now, the below code really ends up being just a no-op.
		 */
	...
	public void sasllogin(String[] allowed, String realm, String authzid, String u, String p) throws ProtocolException {
		if (saslAuthenticator == null) {
			try {
				Class sac = Class.forName(IMAPSaslAuthenticator.class.getName());
				Constructor c = sac.getConstructor(new Class[] { IMAPProtocol.class, String.class, Properties.class, Boolean.TYPE, PrintStream.class, String.class });
				saslAuthenticator = (SaslAuthenticator) c.newInstance(new Object[] { this, name, props, debug ? Boolean.TRUE : Boolean.FALSE, out, host });
			} catch (Exception ex) {
				if (debug)
	...
	public AppendUID appenduid(String mbox, Flags f, Date d, Literal data, boolean uid) throws ProtocolException {
		...
		/*
		 * HACK ALERT: We want the flag_list to be written out without any checking/processing of the bytes in it. If I use writeString(), the flag_list will end up being quoted since it contains "illegal" characters. So I am depending on implementation knowledge that writeAtom() does not do any checking/processing - it just writes out the bytes. What we really need is a writeFoo() that just dumps out its argument.
		 */
	...
	/**
	 * Fetch the specified RFC822 Data item. 'what' names the item to be fetched. 'what' can be <code>null</code> to fetch the whole message.
	 */
	...
	/**
	 * Get the sequence number for the given UID. A UID object containing the sequence number is returned. If the given UID is invalid, <code>null</code> is returned.
	 */
	...
	/*
	 * Apply the given SearchTerm on the specified sequence. Returns array of matching sequence numbers. Note that an empty array is returned for no matches.
	 */
	private int[] search(String msgSequence, SearchTerm term) throws ProtocolException, SearchException {
		...
		/*
		 * The search "text" terms do contain non-ASCII chars. We need to use SEARCH CHARSET <charset> ... The charsets we try to use are UTF-8 and the locale's default charset. If the server supports UTF-8, great, always use it. Else we try to use the default charset.
		 */
		...
		for (int i = 0; i < searchCharsets.length; i++) {
			...
			try {
				...
				/*
				 * Server returned NO. For now, I'll just assume that this indicates that this charset is unsupported. We can check the BADCHARSET response code once that's spec'd into the IMAP RFC ..
				 */
	...
	/*
	 * Apply the given SearchTerm on the specified sequence, using the given charset. <p> Returns array of matching sequence numbers. Note that an empty array is returned for no matches.
	 */
	 ...
	 *
	 * Returns an array of Quota objects, representing the quotas for this mailbox and, indirectly, the quotaroots for this mailbox.
	 *
	 * @see "RFC2087"
	 */
	...
	public void setQuota(Quota quota) throws ProtocolException {
		...
		/*
		 * Quota quota = null; Vector v = new Vector();
		 * 
		 * // Grab all QUOTA responses if (response.isOK()) { // command succesful for (int i = 0, len = r.length; i < len; i++) { if (!(r[i] instanceof IMAPResponse)) continue;
		 * 
		 * IMAPResponse ir = (IMAPResponse)r[i]; if (ir.keyEquals("QUOTA")) { quota = parseQuota(ir); v.addElement(quota); r[i] = null; } } }
		 */
