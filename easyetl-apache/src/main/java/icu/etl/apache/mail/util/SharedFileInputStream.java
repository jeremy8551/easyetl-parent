...
//	    }
	 ...
	 *
	 * <p>
	 * This method implements the general contract of the corresponding <code>{@link java.io.InputStream#read(byte[], int, int) read}</code> method of the <code>{@link java.io.InputStream}</code> class.
	 *
	 * @param b
	 *            destination buffer.
	 * @param off
	...
	/**
	 * Tests if this input stream supports the <code>mark</code> and <code>reset</code> methods. The <code>markSupported</code> method of <code>SharedFileInputStream</code> returns <code>true</code>.
	 *
...
//System.out.println("getPosition: start " + start + " pos " + pos + " bufpos " + bufpos + " = " + (bufpos + pos - start));
	...
	// for testing...
	/*
	 * public static void main(String[] argv) throws Exception { SharedFileInputStream is = new SharedFileInputStream(argv[0]); java.util.Random r = new java.util.Random(); int b; while ((b = is.read()) >= 0) { System.out.write(b); if (r.nextDouble() < 0.3) { InputStream is2 = is.newStream(is.getPosition(), -1); int b2; while ((b2 = is2.read()) >= 0) ; } } }
	 */
