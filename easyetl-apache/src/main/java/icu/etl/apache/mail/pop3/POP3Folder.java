...
package icu.etl.apache.mail.pop3;
...
import icu.etl.apache.mail.MethodNotSupportedException;
...
public class POP3Folder extends Folder {
	...
	/**
	 * Always throws <code>MessagingException</code> because no POP3 folders can contain subfolders.
	 *
	...
	/**
	 * Always returns <code>false</code>; the POP3 protocol doesn't support creating folders.
	 *
	 * @return false
	 */
	...
	/**
	 * Always returns <code>false</code>; the POP3 protocol provides no way to determine when a new message arrives.
	 *
	 * @return false
	 */
	...
	/**
	 * Always throws <code>MethodNotSupportedException</code> because the POP3 protocol doesn't allow the INBOX to be deleted.
	 *
	...
	/**
	 * Always throws <code>MethodNotSupportedException</code> because the POP3 protocol doesn't support multiple folders.
	 *
	 * @exception MethodNotSupportedException
	 *                always
	 */
	...
	/**
	 * Throws <code>FolderNotFoundException</code> unless this folder is named "INBOX".
	 *
	...
	/**
	 * Always returns an empty <code>Flags</code> object because the POP3 protocol doesn't support any permanent flags.
	 *
	...
	/**
	 * Always throws <code>MethodNotSupportedException</code> because the POP3 protocol doesn't support appending messages.
	 *
	 * @exception MethodNotSupportedException
	 *                always
	 */
	...
	/**
	 * Always throws <code>MethodNotSupportedException</code> because the POP3 protocol doesn't support expunging messages without closing the folder; call the {@link #close close} method with the <code>expunge</code> argument set to <code>true</code> instead.
	 *
	 * @exception MethodNotSupportedException
	 *                always
	 */
	...
	public synchronized void fetch(Message[] msgs, FetchProfile fp) throws MessagingException {
		...
		if (!doneUidl && fp.contains(UIDFolder.FetchProfileItem.UID)) {
			/*
			 * Since the POP3 protocol only lets us fetch the UID for a single message or for all messages, we go ahead and fetch UIDs for all messages here, ignoring the msgs parameter. We could be more intelligent and base this decision on the number of messages fetched, or the percentage of the total number of messages fetched.
			 */
	...
	/**
	 * Return the sizes of all messages in this folder, as returned by the POP3 LIST command. Each entry in the array corresponds to a message; entry <i>i</i> corresponds to message number <i>i+1</i>.
	 *
