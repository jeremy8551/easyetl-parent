...
package icu.etl.apache.mail.iap;
...
public class ResponseInputStream {
	...
	/**
	 * Private method that does the actual read. This method moves data into 'buffer'.
	 */
		...
		if (idx >= 5 && buffer[idx - 3] == '}') {
			...
			if (count > 0) {
				...
				if (count > avail)
					...
					growBuffer(minIncrement > count - avail ? minIncrement : count - avail);
				/*
				 * read() might not return all the bytes in one shot, so call repeatedly till we are done
				 */
				int actual;
				while (count > 0) {
			...
			/*
			 * Recursive call to continue processing till CR-LF .. .. the additional bytes get added into 'buffer'
			 */
			read0();
		}
		return;
