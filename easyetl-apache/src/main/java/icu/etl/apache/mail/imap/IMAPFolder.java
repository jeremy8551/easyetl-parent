...
package icu.etl.apache.mail.imap;
...
import icu.etl.apache.mail.FetchProfile;
import icu.etl.apache.mail.Flags;
import icu.etl.apache.mail.Folder;
import icu.etl.apache.mail.FolderClosedException;
import icu.etl.apache.mail.FolderNotFoundException;
import icu.etl.apache.mail.Message;
import icu.etl.apache.mail.MessageRemovedException;
import icu.etl.apache.mail.MessagingException;
import icu.etl.apache.mail.Quota;
import icu.etl.apache.mail.ReadOnlyFolderException;
import icu.etl.apache.mail.StoreClosedException;
import icu.etl.apache.mail.UIDFolder;
import icu.etl.apache.mail.event.ConnectionEvent;
import icu.etl.apache.mail.event.FolderEvent;
import icu.etl.apache.mail.event.MessageChangedEvent;
import icu.etl.apache.mail.iap.BadCommandException;
import icu.etl.apache.mail.iap.CommandFailedException;
import icu.etl.apache.mail.iap.ConnectionException;
import icu.etl.apache.mail.iap.Literal;
import icu.etl.apache.mail.iap.ProtocolException;
import icu.etl.apache.mail.iap.Response;
import icu.etl.apache.mail.iap.ResponseHandler;
import icu.etl.apache.mail.imap.protocol.FetchResponse;
import icu.etl.apache.mail.imap.protocol.IMAPProtocol;
import icu.etl.apache.mail.imap.protocol.IMAPResponse;
import icu.etl.apache.mail.imap.protocol.ListInfo;
import icu.etl.apache.mail.imap.protocol.MailboxInfo;
import icu.etl.apache.mail.imap.protocol.MessageSet;
import icu.etl.apache.mail.imap.protocol.Status;
import icu.etl.apache.mail.imap.protocol.UID;
import icu.etl.apache.mail.internet.MimeMessage;
import icu.etl.apache.mail.search.FlagTerm;
import icu.etl.apache.mail.search.SearchException;
import icu.etl.apache.mail.search.SearchTerm;
import icu.etl.apache.mail.util.CRLFOutputStream;
 ...
 * <p>
 * <p>
 * Applications that need to make use of IMAP-specific features may cast a <code>Folder</code> object to an <code>IMAPFolder</code> object and use the methods on this class. The {@link #getQuota getQuota} and {@link #setQuota setQuota} methods support the IMAP QUOTA extension. Refer to <A HREF="http://www.ietf.org/rfc/rfc2087.txt">RFC 2087</A> for more information.
 * <p>
 * <p>
 * The {@link #getACL getACL}, {@link #addACL addACL}, {@link #removeACL removeACL}, {@link #addRights addRights}, {@link #removeRights removeRights}, {@link #listRights listRights}, and {@link #myRights myRights} methods support the IMAP ACL extension. Refer to <A HREF="http://www.ietf.org/rfc/rfc2086.txt">RFC 2086</A> for more information.
 * <p>
 * <p>
 ...
 * <p>
 * <p>
 * See the <a href="package-summary.html">com.sun.mail.imap</a> package documentation for further information on the IMAP protocol provider.
 * <p>
 *
 * <strong>WARNING:</strong> The APIs unique to this class should be considered <strong>EXPERIMENTAL</strong>. They may be changed in the future in ways that are incompatible with applications using the current APIs.
 *
 * @author John Mani
 * @author Bill Shannon
 * @author Jim Glennon
...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler {
    ...
    /*
     * An IMAP delimiter is a 7bit US-ASCII character. (except NUL). We use '\uffff' (a non 7bit character) to indicate that we havent yet determined what the separator character is. We use '\u0000' (NUL) to indicate that no separator character exists, i.e., a flat hierarchy
     */
    ...
    /*
     * This field tracks the state of this folder. If the folder is closed due to external causes (i.e, not thru the close() method), then this field will remain false. If the folder is closed thru the close() method, then this field is set to true.
     *
     * If reallyClosed is false, then a FolderClosedException is generated when a method is invoked on any Messaging object owned by this folder. If reallyClosed is true, then the IllegalStateException runtime exception is thrown.
     */
    ...
    /**
     * A fetch profile item for fetching headers. This inner class extends the <code>FetchProfile.Item</code> class to add new FetchProfile item types, specific to IMAPFolders.
     *
     * @see FetchProfile
     */
    public static class FetchProfileItem extends FetchProfile.Item {
        ...
        /**
         * HEADERS is a fetch profile item that can be included in a <code>FetchProfile</code> during a fetch request to a Folder. This item indicates that the headers for messages in the specified range are desired to be prefetched.
         * <p>
         * <p>
        ...
        /**
         * SIZE is a fetch profile item that can be included in a <code>FetchProfile</code> during a fetch request to a Folder. This item indicates that the sizes of the messages in the specified range are desired to be prefetched.
         * <p>
         * <p>
     ...
     * @param separator the default separator character for this folder's namespace
    ...
    /*
     * Ensure that this folder exists. If 'exists' has been set to true, we don't attempt to validate it with the server again. Note that this can result in a possible loss of sync with the server.
     */
    ...
    /*
     * Ensure the folder is closed. ASSERT: Must be called with this folder's synchronization lock held.
     */
    ...
    /*
     * Ensure the folder is open. ASSERT: Must be called with this folder's synchronization lock held.
     */
    ...
    /*
     * Check that the given message number is within the range of messages present in this folder. If the message number is out of range, we ping the server to obtain any pending new message notifications from the server.
     */
    ...
    /*
     * Check whether the given flags are supported by this server, and also verify that the folder allows setting flags.
     */
    ...
    public String getName() {
        /*
         * Return the last component of this Folder's full name. Folder components are delimited by the separator character.
         */
        if (name == null) {
            try {
    ...
    /**
     * Which entry in <code>li</code> matches <code>lname</code>? If the name contains wildcards, more than one entry may be returned.
     */
    ...
    private Folder[] doList(final String pattern, final boolean subscribed) throws MessagingException {
        ...
        /*
         * The UW based IMAP4 servers (e.g. SIMS2.0) include current folder (terminated with the separator), when the LIST pattern is '%' or '*'. i.e, <LIST "" mail/%> returns "mail/" as the first LIST response.
         *
         * Doesn't make sense to include the current folder in this case, so we filter it out. Note that I'm assuming that the offending response is the *first* one, my experiments with the UW & SIMS2.0 servers indicate that ..
         */
        int start = 0;
    ...
    public synchronized void open(int mode) throws MessagingException {
        ...
        synchronized (messageCacheLock) { // Acquire messageCacheLock
            ...
            /*
             * Add response handler right away so we get any alerts or notifications that occur during the SELECT or EXAMINE. Have to be sure to remove it if we fail to open the folder.
             */
        ...
        /*
         * Handle SELECT or EXAMINE failure after lock is released. Try to figure out why the operation failed so we can report a more reasonable exception.
         */
        if (exc != null) {
    ...
    private void close(boolean expunge, boolean force) throws MessagingException {
        synchronized (messageCacheLock) {
            /*
             * If we already know we're closed, this is illegal. Can't use checkOpened() because if we were forcibly closed asynchronously we just want to complete the closing here.
             */
...
//                            MailboxInfo mi =
    ...
    /*
     * Get results of STATUS command for this folder, checking cache first. ASSERT: Must be called with this folder's synchronization lock held. ASSERT: The folder must be closed.
     */
    private Status getStatus() throws ProtocolException {
        ...
        if (statusCacheTimeout > 0 && cachedStatus != null && System.currentTimeMillis() - cachedStatusTime < statusCacheTimeout)
            return cachedStatus;
    ...
    /**
     * Append the given messages into this folder. Return array of AppendUID objects containing UIDs of these messages in the destination folder. Each element of the returned array corresponds to an element of the <code>msgs</code> array. A null element means the server didn't return UID information for the appended message.
     * <p>
     * <p>
     * Depends on the APPENDUID response code defined by the UIDPLUS extension - <A HREF="http://www.ietf.org/rfc/rfc2359.txt">RFC 2359</A>.
     */
    ...
    /**
     * Append the given messages into this folder. Return array of Message objects representing the messages in the destination folder. Note that the folder must be open. Each element of the returned array corresponds to an element of the <code>msgs</code> array. A null element means the server didn't return UID information for the appended message.
     * <p>
     * <p>
     * Depends on the APPENDUID response code defined by the UIDPLUS extension - <A HREF="http://www.ietf.org/rfc/rfc2359.txt">RFC 2359</A>.
     */
        ...
        synchronized (messageCacheLock) {
            ...
            for (int i = 0; i < messageCache.size(); ) {
                ...
                if (m.isExpunged()) {
                     ...
                     *
                     * Note that this also causes all succeeding messages in the cache to shifted downward in the vector, therby decrementing the vector's size. (and hence we need to do messageCache.size() at the top of this loop.
                     */
    ...
    /**
     * Get the Message corresponding to the given UID. If no such message exists, <code> null </code> is returned.
     */
     ...
     * <p>
     *
     * <code>uids.length()</code> elements are returned. If any UID in the array is invalid, a <code>null</code> entry is returned for that element.
     */
    ...
    /**
     * Set the quotas for the quotaroot specified in the quota argument. Typically this will be one of the quotaroots associated with this folder, as obtained from the <code>getQuota</code> method, but it need not be.
     *
     ...
     * <p>
     * <p>
     * Note that this method lists the rights that it is possible to assign to the given identifier, <em>not</em> the rights that are actually granted to the given identifier. For the latter, see the <code>getACL</code> method.
     *
     ...
     */
    /*
     * ASSERT: This method must be called only when holding the messageCacheLock. ASSERT: This method must *not* invoke any other method that might grab the 'folder' lock or 'message' lock (i.e., any synchronized methods on IMAPFolder or IMAPMessage) since that will result in violating the locking hierarchy.
     */
     ...
     * <p>
     * When acquiring a store protocol object, it is important to use the following steps:
     * <p>
     * IMAPProtocol p = null; try { p = getStoreProtocol(); // perform the command } catch (WhateverException ex) { // handle it } finally { releaseStoreProtocol(p); }
     */
     ...
     * <p>
     *
     * <strong>NOTE:</strong> This method was intended to allow experimentation with simple extension commands that can use the low level Protocol object APIs to send commands and process responses.
     * <p>
     *
     * <strong>NOTE:</strong> Using this protocol object is completely <strong>UNSAFE</strong> because there's no way to aquire the required locks. See the <code>doCommand</code> method for a safe alternative.
     * <p>
     *
    ...
    /**
	 * Execute a user-supplied IMAP command. The command is executed in the appropriate context with the necessary locks held and using the appropriate <code>IMAPProtocol</code> object.
	 * <p>
	 * <p>
	 * This method returns whatever the <code>ProtocolCommand</code> object's <code>doCommand</code> method returns. If the <code>doCommand</code> method throws a <code>ConnectionException</code> it is translated into a <code>StoreClosedException</code> or <code>FolderClosedException</code> as appropriate. If the <code>doCommand</code> method throws a <code>ProtocolException</code> it is translated into a <code>MessagingException</code>.
	 * <p>
	 * <p>
	 * The following example shows how to execute the IMAP NOOP command. Executing more complex IMAP commands requires intimate knowledge of the <code>com.sun.mail.iap</code> and <code>com.sun.mail.imap.protocol</code> packages, best acquired by reading the source code.
	 * <p>
	 *
	 * <blockquote>
	 *
    ...
    /**
     * Issue a noop command for the connection if the connection has not been used in more than a second. If <code>keepStoreAlive</code> is true, also issue a noop over the store's connection.
     */
    ...
    /**
     * Get the message object for the given sequence number. If none found, null is returned.
     * <p>
     * ASSERT: This method must be called only when holding the messageCacheLock
     */
    IMAPMessage getMessageBySeqNumber(int seqnum) {
        /*
         * Check messageCache for message matching the given sequence number. We start the search from position (seqnum-1) and continue down the vector till we get a match.
         */
