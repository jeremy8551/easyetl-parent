 ...
 * <p>
 *
 * A System flag is represented by the <code>Flags.Flag</code> inner class. A User defined flag is represented as a String. User flags are case-independent.
 * <p>
 *
 * A set of standard system flags are predefined. Most folder implementations are expected to support these flags. Some implementations may also support arbitrary user-defined flags. The <code>getPermanentFlags</code> method on a Folder returns a Flags object that holds all the flags that are supported by that folder implementation.
 * <p>
 *
 ...
 * <p>
 *
 * <strong>Warning:</strong> Serialized objects of this class may not be compatible with future JavaMail API releases. The current serialization support is appropriate for short term storage.
 * <p>
 *
...
public class Flags implements Cloneable, Serializable {
	...
	public static final class Flag {
		...
		/**
		 * This message is seen. This flag is implicitly set by the implementation when the this Message's content is returned to the client in some form. The <code>getInputStream</code> and <code>getContent</code> methods on Message cause this flag to be set.
		 * <p>
		 *
		 ...
		 * <p>
		 *
		 * The implementation sets this flag. Clients cannot alter this flag but can use it to determine if a folder supports user defined flags by using <code>folder.getPermanentFlags().contains(Flags.Flag.USER)</code>.
		 */
	 ...
	 *
	 * @return true if all flags in the given Flags object are present, otherwise false.
	 */
	...
	/*****
	 * public static void main(String argv[]) throws Exception { // a new flags object Flags f1 = new Flags(); f1.add(Flags.Flag.DELETED); f1.add(Flags.Flag.SEEN); f1.add(Flags.Flag.RECENT); f1.add(Flags.Flag.ANSWERED);
	 * 
	 * // check copy constructor with only system flags Flags fc = new Flags(f1); if (f1.equals(fc) && fc.equals(f1)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * // check clone with only system flags fc = (Flags)f1.clone(); if (f1.equals(fc) && fc.equals(f1)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * // add a user flag and make sure it still works right f1.add("MyFlag");
	 * 
	 * // shouldn't be equal here if (!f1.equals(fc) && !fc.equals(f1)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * // check clone fc = (Flags)f1.clone(); if (f1.equals(fc) && fc.equals(f1)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * // make sure user flag hash tables are separate fc.add("AnotherFlag"); if (!f1.equals(fc) && !fc.equals(f1)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * // check copy constructor fc = new Flags(f1); if (f1.equals(fc) && fc.equals(f1)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * // another new flags object Flags f2 = new Flags(Flags.Flag.ANSWERED); f2.add("MyFlag");
	 * 
	 * if (f1.contains(Flags.Flag.DELETED)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * if (f1.contains(Flags.Flag.SEEN)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * if (f1.contains(Flags.Flag.RECENT)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * if (f1.contains("MyFlag")) System.out.println("success"); else System.out.println("fail");
	 * 
	 * if (f2.contains(Flags.Flag.ANSWERED)) System.out.println("success"); else System.out.println("fail");
	 * 
	 * 
	 * System.out.println("----------------");
	 * 
	 * String[] s = f1.getUserFlags(); for (int i = 0; i < s.length; i++) System.out.println(s[i]); System.out.println("----------------"); s = f2.getUserFlags(); for (int i = 0; i < s.length; i++) System.out.println(s[i]);
	 * 
	 * System.out.println("----------------");
	 * 
	 * if (f1.contains(f2)) // this should be true System.out.println("success"); else System.out.println("fail");
	 * 
	 * if (!f2.contains(f1)) // this should be false System.out.println("success"); else System.out.println("fail");
	 * 
	 * Flags f3 = new Flags(); f3.add(Flags.Flag.DELETED); f3.add(Flags.Flag.SEEN); f3.add(Flags.Flag.RECENT); f3.add(Flags.Flag.ANSWERED); f3.add("ANOTHERFLAG"); f3.add("MYFLAG");
	 * 
	 * f1.add("AnotherFlag");
	 * 
	 * if (f1.equals(f3)) System.out.println("equals success"); else System.out.println("fail"); if (f3.equals(f1)) System.out.println("equals success"); else System.out.println("fail"); System.out.println("f1 hash code " + f1.hashCode()); System.out.println("f3 hash code " + f3.hashCode()); if (f1.hashCode() == f3.hashCode()) System.out.println("success"); else System.out.println("fail"); }
	 ****/
}
