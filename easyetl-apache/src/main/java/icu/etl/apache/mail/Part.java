...
/**
 * The <code>Part</code> interface is the common base interface for Messages and BodyParts.
 * <p>
 *
 ...
 * <p>
 *
 * The <strong>data type</strong> of the "content" is returned by the <code>getContentType()</code> method. The MIME typing system is used to name data types.
 * <p>
 *
 * The "content" of a Part is available in various formats:
 * <ul>
 * <li>As a DataHandler - using the <code>getDataHandler()</code> method. The "content" of a Part is also available through a <code>javax.activation.DataHandler</code> object. The DataHandler object allows clients to discover the operations available on the content, and to instantiate the appropriate component to perform those operations.
 *
 * <li>As an input stream - using the <code>getInputStream()</code> method. Any mail-specific encodings are decoded before this stream is returned.
 *
 * <li>As a Java object - using the <code>getContent()</code> method. This method returns the "content" as a Java object. The returned object is of course dependent on the content itself. In particular, a "multipart" Part's content is always a Multipart or subclass thereof. That is, <code>getContent()</code> on a "multipart" type Part will always return a Multipart (or subclass) object.
 * </ul>
 *
 * Part provides the <code>writeTo()</code> method that streams out its bytestream in mail-safe form suitable for transmission. This bytestream is typically an aggregation of the Part attributes and its content's bytestream.
 * <p>
 *
...
public interface Part {
	...
	/**
	 * Return the number of lines in the content of this part. Return -1 if the number cannot be determined.
	 *
	 ...
	 *                if the underlying implementation does not support modification of this header
	 ...
	 * <p>
	 *
	 * This is typically a convenience method that just invokes the DataHandler's <code>getInputStream()</code> method.
	 *
	 ...
	 * <p>
	 *
	 * Note that a DataContentHandler class for the specified type should be available to the JavaMail implementation for this to work right. i.e., to do <code>setContent(foobar, "application/x-foobar")</code>, a DataContentHandler for "application/x-foobar" should be installed. Refer to the Java Activation Framework for more information.
	 *
	 * @param obj
	 *            A java object.
	 * @param type
	...
	/**
	 * Get all the headers for this header name. Returns <code>null</code> if no headers for this header name are available.
	 *
	 * @param header_name
