...
import icu.etl.apache.mail.Header;
...
import icu.etl.apache.mail.imap.protocol.BODY;
import icu.etl.apache.mail.imap.protocol.BODYSTRUCTURE;
import icu.etl.apache.mail.imap.protocol.ENVELOPE;
import icu.etl.apache.mail.imap.protocol.FetchResponse;
import icu.etl.apache.mail.imap.protocol.IMAPProtocol;
import icu.etl.apache.mail.imap.protocol.INTERNALDATE;
import icu.etl.apache.mail.imap.protocol.Item;
import icu.etl.apache.mail.imap.protocol.MessageSet;
import icu.etl.apache.mail.imap.protocol.RFC822DATA;
import icu.etl.apache.mail.imap.protocol.RFC822SIZE;
import icu.etl.apache.mail.imap.protocol.UID;
import icu.etl.apache.mail.internet.ContentType;
import icu.etl.apache.mail.internet.InternetAddress;
import icu.etl.apache.mail.internet.InternetHeaders;
 ...
 */
/*
 * The lock hierarchy is that the lock on the IMAPMessage object, if it's acquired at all, must be acquired before the message cache lock. The IMAPMessage lock protects the message flags, sort of.
 *
 * XXX - I'm not convinced that all fields of IMAPMessage are properly protected by locks.
 */
...
public class IMAPMessage extends MimeMessage {
	...
	/*
	 * Hashtable of names of headers we've loaded from the server. Used in isHeaderLoaded() and setHeaderLoaded() to keep track of those headers we've attempted to load from the server. We need this table of names to avoid multiple attempts at loading headers that don't exist for a particular message.
	 *
	...
	/**
	 * Get this message's folder's protocol connection. Throws FolderClosedException, if the protocol connection is not available.
	 *
	...
	public synchronized DataHandler getDataHandler() throws MessagingException {
		...
		if (dh == null) {
			...
			/*
			 * Special-case Multipart and Nested content. All other cases are handled by the superclass.
			 */
			if (bs.isMulti())
				dh = new DataHandler(new IMAPMultipartDataSource(this, bs.bodies, sectionId, this));
			else if (bs.isNested() && getProtocol().isREV1())
				/*
				 * Nested messages are handled specially only for IMAP4rev1. IMAP4 doesn't provide enough support to FETCH the components of nested messages
				 */
				dh = new DataHandler(new IMAPNestedMessage(this, bs.bodies[0], bs.envelope, sectionId == null ? "1" : sectionId + ".1"), type);
		}
				...
				for (int j = 0; j < count; j++) {
					...
					// Check for header items
						...
						if (msg.headers == null || allHeaders)
							...
							/*
							 * This is really painful. A second fetch of the same headers (which might occur because a new header was added to the set requested) will return headers we already know about. In this case, only load the headers we haven't seen before to avoid adding duplicates of headers we already have.
							 */
	...
	private boolean isHeaderLoaded(String name) {
		...
		return (loadedHeaders != null) ? loadedHeaders.containsKey(name.toUpperCase()) : false;
	}
	...
	/*
	 * Convert the given FETCH item identifier to the approriate section-string for this message.
	 */
