...
import icu.etl.apache.mail.activation.FileDataSource;
import icu.etl.apache.mail.BodyPart;
...
/**
 * This class represents a MIME body part. It implements the <code>BodyPart</code> abstract class and the <code>MimePart</code> interface. MimeBodyParts are contained in <code>MimeMultipart</code> objects.
 * <p>
 *
 * MimeBodyPart uses the <code>InternetHeaders</code> class to parse and store the headers of that body part.
 * <p>
 *
 * <hr>
...
public class MimeBodyPart extends BodyPart implements MimePart {
	...
	/**
	 * If the data for this body part was supplied by an InputStream that implements the SharedInputStream interface, <code>contentStream</code> is another such stream representing the content of this body part. In this case, <code>content</code> will be null.
	 *
	 * @since JavaMail 1.2
	 */
	 ...
	 * <p>
	 *
	 * Note that the "boundary" string that delimits body parts must <strong>not</strong> be included in the input stream. The intention is that the MimeMultipart parser will extract each body part's bytes from a multipart stream and feed them into this constructor, without the delimiter strings.
	 *
	 * @param is
	 ...
	 * <p>
	 *
	 * This implementation uses <code>getHeader(name)</code> to obtain the requisite header field.
	 *
	...
	/**
	 * Set the "Content-ID" header field of this body part. If the <code>cid</code> parameter is null, any existing "Content-ID" is removed.
	 *
	 * @exception IllegalWriteException
	 *                if the underlying implementation does not support modification
	 * @exception IllegalStateException
	...
	/**
	 * Get the languages specified in the Content-Language header of this MimePart. The Content-Language header is defined by RFC 1766. Returns <code>null</code> if this header is not available or its value is absent.
	 * <p>
	 *
	 * This implementation uses <code>getHeader(name)</code> to obtain the requisite header field.
	 */
	...
	/**
	 * Set the "Content-Description" header field for this body part. If the description parameter is <code>null</code>, then any existing "Content-Description" fields are removed.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * Returns the value of the "filename" parameter from the "Content-Disposition" header field of this body part. If its not available, returns the value of the "name" parameter from the "Content-Type" header field of this body part. Returns <code>null</code> if both are absent.
	 * <p>
	 *
	...
	/**
	 * Examine the content of this body part and update the appropriate MIME headers. Typical headers that get set here are <code>Content-Type</code> and <code>Content-Transfer-Encoding</code>. Headers might need to be updated in two cases:
	 *
	 * <br>
	 ...
	 *
	 * <br>
	 * In both cases this method is typically called by the <code>Message.saveChanges</code> method.
	 */
	...
	static void setFileName(MimePart part, String name) throws MessagingException {
		...
		/*
		 * Also attempt to set the Content-Type "name" parameter, to satisfy ancient MUAs. XXX - This is not RFC compliant.
		 */
	...
	static void updateHeaders(MimePart part) throws MessagingException {
		...
		try {
			...
			if (needCTHeader) {
				/*
				 * Pull out "filename" from Content-Disposition, and use that to set the "name" parameter. This is to satisfy older MUAs (DtMail, Roam and probably a bunch of others).
				 */
				String s = part.getHeader("Content-Disposition", null);
				if (s != null) {
					// Parse the header ..
...
/**
 * A package-private class that serves just as a "flag" that we're using a special data handler to cache getContent data.
 */
