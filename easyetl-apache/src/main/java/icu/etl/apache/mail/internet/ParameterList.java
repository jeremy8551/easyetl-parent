...
/**
 * This class holds MIME parameters (attribute-value pairs). The <code>mail.mime.encodeparameters</code> and <code>mail.mime.decodeparameters</code> System properties control whether encoded parameters, as specified by <a href="http://www.ietf.org/rfc/rfc2231.txt">RFC 2231</a>, are supported. By default, such encoded parameters are not supported.
 * <p>
 *
 * Also, in the current implementation, setting the System property <code>mail.mime.decodeparameters.strict</code> to <code>"true"</code> will cause a <code>ParseException</code> to be thrown for errors detected while decoding encoded parameters. By default, if any decoding errors occur, the original (undecoded) string is used.
 *
...
public class ParameterList {
	...
	/**
	 * The map of name, value pairs. The value object is either a String, for unencoded values, or a Value object, for encoded values.
	 */
	 ...
	 * @param name
	 *            parameter name.
	 * @return Value of the parameter. Returns <code>null</code> if the parameter is not present.
	 */
	...
	/**
	 * Set a parameter. If this parameter already exists, it is replaced by this new value. If the <code>mail.mime.encodeparameters</code> System property is true, and the parameter value is non-ASCII, it will be encoded with the specified charset, as specified by RFC 2231.
	 *
	 * @param name
	 *            name of the parameter.
	 * @param value
	 ...
	 * Convert this ParameterList into a MIME String. If this is an empty list, an empty string is returned.
	 * 
	 * The 'used' parameter specifies the number of character positions already taken up in the field into which the resulting parameter list is to be inserted. It's used to determine where to fold the resulting parameter list.
	 *
	 * @param used
	 *            number of character positions already used, in the field into which the parameter list is to be inserted.
	 * @return String
	 */
	...
	/**
	 * Encode a parameter value, if necessary. If the value is encoded, a Value object is returned. Otherwise, null is returned.
	 */
...
//	    String lang = value.substring(i + 1, li);
			...
			/*
			 * Decode the ASCII characters in value into an array of bytes, and then convert the bytes to a String using the specified charset. We'll never need more bytes than encoded characters, so use that to size the array.
			 */
