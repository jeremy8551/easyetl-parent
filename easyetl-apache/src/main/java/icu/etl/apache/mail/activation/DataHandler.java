...
package icu.etl.apache.mail.activation;
 ...
 * <p>
 * The DataHandler keeps track of the current CommandMap that it uses to service requests for commands (<code>getCommand</code>, <code>getAllCommands</code>, <code>getPreferredCommands</code>). Each instance of a DataHandler may have a CommandMap associated with it using the <code>setCommandMap</code> method. If a CommandMap was not set, DataHandler calls the <code>getDefaultCommandMap</code> method in CommandMap and uses the value it returns. See <i>CommandMap</i> for more information.
 * <p>
 *
...
public class DataHandler implements Transferable {
	...
	/**
	 * Create a <code>DataHandler</code> instance referencing the specified DataSource. The data exists in a byte stream form. The DataSource will provide an InputStream to access the data.
	 *
	 * @param ds
	 *            the DataSource
	 */
	...
	/**
	 * Create a <code>DataHandler</code> instance representing an object of this MIME type. This constructor is used when the application already has an in-memory representation of the data in the form of a Java Object.
	 *
	 * @param obj
	 *            the Java Object
	 * @param mimeType
	...
	/**
	 * Create a <code>DataHandler</code> instance referencing a URL. The DataHandler internally creates a <code>URLDataSource</code> instance to represent the URL.
	 *
	 * @param url
	 *            a URL object
	 */
	...
	/**
	 * Return the name of the data object. If this DataHandler was created with a DataSource, this method calls through to the <code>DataSource.getName</code> method, otherwise it returns <i>null</i>.
	 *
	...
	/**
	 * Return the MIME type of this object as retrieved from the source object. Note that this is the <i>full</i> type with parameters.
	 *
	 ...
	 * <p>
	 *
	 * For DataHandlers instantiated with a DataSource, the DataHandler calls the <code>DataSource.getInputStream</code> method and returns the result to the caller.
	 * <p>
	 ...
	 * <p>
	 * If the DataHandler was created with an object, writeTo retrieves the DataContentHandler for the object's type. If the DataContentHandler was found, it calls the <code>writeTo</code> method on the <code>DataContentHandler</code>.
	 *
	 * @param os
	...
	/**
	 * Get an OutputStream for this DataHandler to allow overwriting the underlying data. If the DataHandler was created with a DataSource, the DataSource's <code>getOutputStream</code> method is called. Otherwise, <code>null</code> is returned.
	 *
	 ...
	 * <p>
	 *
	 * The DataHandler attempts to find a DataContentHandler that corresponds to the MIME type of the data. If one is located, the DataHandler calls the DataContentHandler's <code>getTransferDataFlavors</code> method.
	 * <p>
	 *
	 * If a DataContentHandler can <i>not</i> be located, and if the DataHandler was created with a DataSource (or URL), one DataFlavor is returned that represents this object's MIME type and the <code>java.io.InputStream</code> class. If the DataHandler was created with an object and a MIME type, getTransferDataFlavors returns one DataFlavor that represents this object's MIME type and the object's class.
	 *
	 ...
	 * <p>
	 *
	 * This method iterates through the DataFlavors returned from <code>getTransferDataFlavors</code>, comparing each with the specified flavor.
	 *
	 * @param flavor
	 ...
	 * <p>
	 *
	 * The DataHandler attempts to locate a DataContentHandler for this MIME type. If one is found, the passed in DataFlavor and the type of the data are passed to its <code>getTransferData</code> method. If the DataHandler fails to locate a DataContentHandler and the flavor specifies this object's MIME type and the <code>java.io.InputStream</code> class, this object's InputStream is returned. Otherwise it throws an UnsupportedFlavorException.
	 * <p>
	 *
	...
	/**
	 * Set the CommandMap for use by this DataHandler. Setting it to <code>null</code> causes the CommandMap to revert to the CommandMap returned by the <code>CommandMap.getDefaultCommandMap</code> method. Changing the CommandMap, or setting it to <code>null</code>, clears out any data cached from the previous CommandMap.
	 *
	 * @param commandMap
	...
	/**
	 * Return all the commands for this type of data. This method returns an array containing all commands for the type of data represented by this DataHandler. The MIME type for the underlying data represented by this DataHandler is used to call through to the <code>getAllCommands</code> method of the CommandMap associated with this DataHandler.
	 *
	...
	/**
	 * Get the command <i>cmdName</i>. Use the search semantics as defined by the CommandMap installed in this DataHandler. The MIME type for the underlying data represented by this DataHandler is used to call through to the <code>getCommand</code> method of the CommandMap associated with this DataHandler.
	 *
	 * @param cmdName
	 *            the command name
	 ...
	 * <p>
	 *
	 * If the DataHandler was instantiated with a DataSource, this method uses a DataContentHandler to return the content object for the data represented by this DataHandler. If no <code>DataContentHandler</code> can be found for the the type of this data, the DataHandler returns an InputStream for the data.
	 *
	 * @return the content.
	 * @exception IOException
	 ...
	 * <p>
	 * This method calls the CommandInfo's <code>getCommandObject</code> method with the <code>ClassLoader</code> used to load the <code>javax.activation.DataHandler</code> class itself.
	 *
	 * @param cmdinfo
 ...
 *
 * This is a <i>private</i> DataContentHandler that wraps the real DataContentHandler in the case where the DataHandler was instantiated with a DataSource.
 */
 ...
 *
 * This is a <i>private</i> DataContentHandler that wraps the real DataContentHandler in the case where the DataHandler was instantiated with an object.
 */
