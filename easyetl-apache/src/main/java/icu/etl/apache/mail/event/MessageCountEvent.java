...
package icu.etl.apache.mail.event;
 ...
 * <p>
 *
 * Note that some folder types may only deliver MessageCountEvents at certain times or after certain operations. IMAP in particular will only notify the client of MessageCountEvents when a client issues a new command. Refer to RFC 2060 <A HREF="http://www.ietf.org/rfc/rfc2060.txt"> http://www.ietf.org/rfc/rfc2060.txt</A> for details. A client may want "poll" the folder by occasionally calling the <code>getMessageCount</code> or <code>isConnected</code> methods to solicit any such notifications.
 *
 * @author John Mani
 */
public class MessageCountEvent extends MailEvent {
	...
	/**
	 * Indicates whether this event is the result of an explicit expunge by this client, or due to an expunge from external sources. If <code>true</code>, this event is due to an explicit expunge and hence all remaining messages in this folder have been renumbered. If <code>false</code>, this event is due to an external expunge.
	 * <p>
	 *
	 * Note that this method is valid only if the type of this event is <code>REMOVED</code>
	 */
