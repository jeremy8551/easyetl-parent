...
import icu.etl.apache.mail.activation.DataSource;
import icu.etl.apache.mail.util.ASCIIUtility;
import icu.etl.apache.mail.util.BASE64DecoderStream;
import icu.etl.apache.mail.util.BASE64EncoderStream;
import icu.etl.apache.mail.util.BEncoderStream;
import icu.etl.apache.mail.util.LineInputStream;
import icu.etl.apache.mail.util.QDecoderStream;
import icu.etl.apache.mail.util.QEncoderStream;
import icu.etl.apache.mail.util.QPDecoderStream;
import icu.etl.apache.mail.util.QPEncoderStream;
import icu.etl.apache.mail.util.UUDecoderStream;
import icu.etl.apache.mail.util.UUEncoderStream;
 ...
 * <p>
 *
 * RFC 822 mail headers <strong>must</strong> contain only US-ASCII characters. Headers that contain non US-ASCII characters must be encoded so that they contain only US-ASCII characters. Basically, this process involves using either BASE64 or QP to encode certain characters. RFC 2047 describes this in detail.
 * <p>
 *
 ...
 * <p>
 * 
 * The <code>setHeader</code> and <code>addHeader</code> methods on MimeMessage and MimeBodyPart assume that the given header values are Unicode strings that contain only US-ASCII characters. Hence the callers of those methods must insure that the values they pass do not contain non US-ASCII characters. The methods in this class help do this.
 * <p>
 *
 * The <code>getHeader</code> family of methods on MimeMessage and MimeBodyPart return the raw header value. These might be encoded as per RFC 2047, and if so, must be decoded into Unicode Strings. The methods in this class help to do this.
 * <p>
 *
...
public class MimeUtility {
	 ...
	 * <ul>
	 * <li>If the primary type of this datasource is "text" and if all the bytes in its input stream are US-ASCII, then the encoding is "7bit". If more than half of the bytes are non-US-ASCII, then the encoding is "base64". If less than half of the bytes are non-US-ASCII, then the encoding is "quoted-printable".
	 * <li>If the primary type of this datasource is not "text", then if all the bytes of its input stream are US-ASCII, the encoding is "7bit". If there is even one non-US-ASCII character, the encoding is "base64".
	 * </ul>
	 *
	 * @param ds
	 *            DataSource
	...
	public static String getEncoding(DataHandler dh) {
		...
		/*
		 * Try to pick the most efficient means of determining the encoding. If this DataHandler was created using a DataSource, the getEncoding(DataSource) method is typically faster. If the DataHandler was created with an object, this method is much faster. To distinguish the two cases, we use a heuristic. A DataHandler created with an object will always have a null name. A DataHandler created with a DataSource will usually have a non-null name.
		 *
		 * XXX - This is actually quite a disgusting hack, but it makes a common case run over twice as fast.
		 */
		if (dh.getName() != null)
	 ...
	 *            the header value
	 * @param charset
	 *            the charset. If this parameter is null, the platform's default chatset is used.
	 * @param encoding
	 *            the encoding to be used. Currently supported values are "B" and "Q". If this parameter is null, then the "Q" encoding is used if most of characters to be encoded are in the ASCII charset, otherwise "B" encoding is used.
	 * @return Unicode string containing only US-ASCII characters
	 */
	...
	public static String decodeText(String etext) throws UnsupportedEncodingException {
		/*
		 * We look for sequences separated by "linear-white-space". (as per RFC 2047, Section 6.1.1) RFC 822 defines "linear-white-space" as SPACE | HT | CR | NL.
		 */
		String lwsp = " \t\n\r";
		StringTokenizer st;
		
		/*
		 * First, lets do a quick run thru the string and check whether the sequence "=?" exists at all. If none exists, we know there are no encoded-words in here and we can just return the string as-is, without suffering thru the later decoding logic. This handles the most common case of unencoded headers efficiently.
		 */
		if (etext.indexOf("=?") == -1)
			return etext;
	...
	/*
	 * Encode the given string. The parameter 'encodingWord' should be true if a RFC 822 "word" token is being encoded and false if a RFC 822 "text" token is being encoded. This is because the "Q" encoding defined in RFC 2047 has more restrictions when encoding "word" tokens. (Sigh)
	 */
	 ...
	 * @return the Java charset equivalent. If a suitable mapping is not available, the passed in charset is itself returned.
	 */
	...
	/**
	 * Get the default charset corresponding to the system's current default locale. If the System property <code>mail.mime.charset</code> is set, a system charset corresponding to this MIME charset will be returned.
	 * <p>
	 * 
	 ...
	 * @param s
	 *            string
	 * @return ALL_ASCII if all characters in the string belong to the US-ASCII charset. MOSTLY_ASCII if more than half of the available characters are US-ASCII characters. Else MOSTLY_NONASCII.
	 */
	...
	/**
	 * Check if the given input stream contains non US-ASCII characters. Upto <code>max</code> bytes are checked. If <code>max</code> is set to <code>ALL</code>, then all the bytes available in this input stream are checked. If <code>breakOnNonAscii</code> is true the check terminates when the first non-US-ASCII character is found and MOSTLY_NONASCII is returned. Else, the check continues till <code>max</code> bytes or till the end of stream.
	 *
	 * @param is
	 *            the input stream
	 * @param max
	 *            maximum bytes to check for. The special value ALL indicates that all the bytes in this input stream must be checked.
	 * @param breakOnNonAscii
	 *            if <code>true</code>, then terminate the the check when the first non-US-ASCII character is found.
	 * @return ALL_ASCII if all characters in the string belong to the US-ASCII charset. MOSTLY_ASCII if more than half of the available characters are US-ASCII characters. Else MOSTLY_NONASCII.
	 */
