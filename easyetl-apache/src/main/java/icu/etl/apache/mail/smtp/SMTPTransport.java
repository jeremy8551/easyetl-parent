...
package icu.etl.apache.mail.smtp;
...
import icu.etl.apache.mail.event.TransportEvent;
import icu.etl.apache.mail.Address;
import icu.etl.apache.mail.Message;
import icu.etl.apache.mail.MessagingException;
import icu.etl.apache.mail.SendFailedException;
import icu.etl.apache.mail.Session;
import icu.etl.apache.mail.Transport;
import icu.etl.apache.mail.URLName;
import icu.etl.apache.mail.internet.InternetAddress;
import icu.etl.apache.mail.internet.MimeMessage;
import icu.etl.apache.mail.internet.MimeMultipart;
import icu.etl.apache.mail.internet.MimePart;
import icu.etl.apache.mail.internet.ParseException;
...
import icu.etl.apache.mail.util.SocketFetcher;
import icu.etl.apache.mail.util.TraceInputStream;
import icu.etl.apache.mail.util.TraceOutputStream;

/**
 * This class implements the Transport abstract class using SMTP for message submission and transport.
 *
 * See the <a href="package-summary.html">com.sun.mail.smtp</a> package documentation for further information on the SMTP protocol provider.
 * <p>
 *
 * @author Max Spivak
 * @author Bill Shannon
...
public class SMTPTransport extends Transport {
	 ...
	 * <p>
	 *
	 * Unless mail.smtp.ehlo is set to false, we'll try to identify ourselves using the ESMTP command EHLO.
	 *
	 * If mail.smtp.auth is set to true, we insist on having a username and password, and will try to authenticate ourselves if the server supports the AUTH extension (RFC 2554).
	 *
	 * @param host
	...
	protected boolean protocolConnect(String host, int port, String user, String passwd) throws MessagingException {
		...
		/*
		 * If mail.smtp.auth is set, make sure we have a valid username and password, even if we might not end up using it (e.g., because the server doesn't support ESMTP or doesn't support the AUTH extension).
		 */
		if (useAuth && (user == null || passwd == null))
			return false;
		
		/*
		 * If port is not specified, set it to value of mail.smtp.port property if it exists, otherwise default to 25.
		 */
		if (port == -1) {
		...
		if (useStartTLS && supportsExtension("STARTTLS")) {
			startTLS();
			/*
			 * Have to issue another EHLO to update list of extensions supported, especially authentication mechanisms. Don't know if this could ever fail, but we ignore failure.
			 */
			ehlo(getLocalHost());
		}
		...
		if (useAuth && (supportsExtension("AUTH") || supportsExtension("AUTH=LOGIN"))) {
			...
			if (supportsAuthentication("LOGIN") || supportsExtension("AUTH=LOGIN")) {
				...
				/*
				 * A 530 response indicates that the server wants us to issue a STARTTLS command first. Do that and try again.
				 */
				if (resp == 530) {
					startTLS();
				...
				/*
				 * Wrap a BASE64Encoder around a ByteArrayOutputstream to craft b64 encoded username and password strings.
				 *
				 * Also note that unlike the B64 definition in MIME, CRLFs should *not* be inserted during the encoding process. So I use Integer.MAX_VALUE (0x7fffffff (> 1G)) as the bytesPerLine, which should be sufficiently large!
				 */
				try {
	 ...
	 * <p>
	 *
	 * If all the <code>addresses</code> succeed the SMTP check using the <code>RCPT TO:</code> command, we attempt to send the message. A TransportEvent of type MESSAGE_DELIVERED is fired indicating the successful submission of a message to the SMTP host.
	 * <p>
	 *
	 * If some of the <code>addresses</code> fail the SMTP check, and the <code>mail.stmp.sendpartial</code> property is not set, sending is aborted. The TransportEvent of type MESSAGE_NOT_DELIVERED is fired containing the valid and invalid addresses. The SendFailedException is also thrown.
	 * <p>
	 *
	 * If some of the <code>addresses</code> fail the SMTP check, and the <code>mail.stmp.sendpartial</code> property is set to true, the message is sent. The TransportEvent of type MESSAGE_PARTIALLY_DELIVERED is fired containing the valid and invalid addresses. The SMTPSendFailedException is also thrown.
	 * <p>
	 *
	 * MessagingException is thrown if the message can't write out an RFC822-compliant stream using its <code>writeTo</code> method.
	 * <p>
	 *
	 * @param message
	...
	public synchronized void sendMessage(Message message, Address[] addresses) throws MessagingException, SendFailedException {
		...
		try {
			...
			if (sendPartiallyFailed) {
				...
				notifyTransportListeners(TransportEvent.MESSAGE_PARTIALLY_DELIVERED, validSentAddr, validUnsentAddr, invalidAddr, this.message);
	...
	/*
	 * Gets the sender's address in the following order: 0. SMTPMessage.getEnvelopeFrom() 1. mail.smtp.from property 2. From: header in the message 3. system username using the InternetAddress.getLocalAddress() method
	 */
	private void mailFrom() throws MessagingException {
		...
		/*
		 * If an RFC 2554 submitter has been specified, and the server supports the AUTH extension, include the AUTH= element on the MAIL FROM command.
		 */
	...
	/**
	 * Sends each address to the SMTP host and copies it either into the validSentAddr or invalidAddr arrays. Sets the <code>sendFailed</code> flag to true if any addresses failed.
	 */
	/*
	 * success/failure/error possibilities from the RCPT command from rfc821, section 4.3 S: 250, 251 F: 550, 551, 552, 553, 450, 451, 452 E: 500, 501, 503, 421
	 *
	 * and how we map the above error/failure conditions to valid/invalid address vectors that are reported in the thrown exception: invalid addr: 550, 501, 503, 551, 553 valid addr: 552 (quota), 450, 451, 452 (quota), 421 (srvr abort)
	 */
	...
	/**
	 * Send the "data" command to smtp host and return an OutputStream to which the data is to be written.
	 */
	...
	private void openServer() throws MessagingException {
		...
		try {
			...
			if ((r = readServerResponse()) != 220) {
				...
				lineInputStream = null;
				if (debug)
					out.println("DEBUG SMTP: got bad greeting from host \"" + server + "\", port: " + port + ", response: " + r + "\n");
	...
	/**
	 * Sends command <code>cmd</code> to the server terminating it with <code>CRLF</code>.
	 */
	...
	/**
	 * Reads server reponse returning the <code>returnCode</code> as the number. Returns -1 on failure. Sets <code>lastServerResponse</code> and <code>lastReturnCode</code>.
	 */
	...
	/**
	 * Return true if the SMTP server supports the specified service extension. Extensions are reported as results of the EHLO command when connecting to the server. See <A HREF="http://www.ietf.org/rfc/rfc1869.txt">RFC 1869</A> and other RFCs that define specific extensions.
	 *
	 * @param ext
