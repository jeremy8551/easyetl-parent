...
package icu.etl.apache.mail.activation.registries;
...
public class MailcapFile {
	...
	/**
	 * for debugging public static void main(String[] args) throws Exception { Map masterHash = new HashMap(); for (int i = 0; i < args.length; ++i) { System.out.println("Entry " + i + ": " + args[i]); parseLine(args[i], masterHash); }
	 * 
	 * Enumeration types = masterHash.keys(); while (types.hasMoreElements()) { String key = (String)types.nextElement(); System.out.println("MIME Type: " + key);
	 * 
	 * Map commandHash = (Map)masterHash.get(key); Enumeration commands = commandHash.keys(); while (commands.hasMoreElements()) { String command = (String)commands.nextElement(); System.out.println(" Command: " + command);
	 * 
	 * Vector classes = (Vector)commandHash.get(command); for (int i = 0; i < classes.size(); ++i) { System.out.println(" Class: " + (String)classes.elementAt(i)); } }
	 * 
	 * System.out.println(""); } }
	 */
}
