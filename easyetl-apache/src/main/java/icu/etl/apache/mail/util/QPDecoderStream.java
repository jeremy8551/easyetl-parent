...
public class QPDecoderStream extends FilterInputStream {
	...
	public int read() throws IOException {
		...
		if (c == ' ') {
			...
			if (a == '\n') {
				/*
				 * Hmm ... not really confirming QP encoding, but lets allow this as a LF terminated encoded line .. and consider this a soft linebreak and recurse to fetch the next char.
				 */
				return read();
			} else if (a == '\r') {
	...
	/**
	 * Returns the number of bytes that can be read from this input stream without blocking. The QP algorithm does not permit a priori knowledge of the number of bytes after decoding, so this method just invokes the <code>available</code> method of the original input stream.
	 */
	...
	/****
	 * begin TEST program public static void main(String argv[]) throws Exception { FileInputStream infile = new FileInputStream(argv[0]); QPDecoderStream decoder = new QPDecoderStream(infile); int c;
	 * 
	 * while ((c = decoder.read()) != -1) System.out.print((char)c); System.out.println(); } end TEST program
	 ****/
}
