...
import static icu.etl.apache.ant.zip.ZipConstants.DATA_DESCRIPTOR_MIN_VERSION;
import static icu.etl.apache.ant.zip.ZipConstants.DWORD;
import static icu.etl.apache.ant.zip.ZipConstants.INITIAL_VERSION;
import static icu.etl.apache.ant.zip.ZipConstants.SHORT;
import static icu.etl.apache.ant.zip.ZipConstants.WORD;
import static icu.etl.apache.ant.zip.ZipConstants.ZIP64_MAGIC;
import static icu.etl.apache.ant.zip.ZipConstants.ZIP64_MAGIC_SHORT;
import static icu.etl.apache.ant.zip.ZipConstants.ZIP64_MIN_VERSION;
...
public class ZipOutputStream extends FilterOutputStream {
	...
	/*
	 * Apparently Deflater.setInput gets slowed down a lot on Sun JVMs when it gets handed a really big buffer. See https://issues.apache.org/bugzilla/show_bug.cgi?id=45396 Using a buffer size of 8 kB proved to be a good compromise
	 */
	 ...
	 *     if the archive's size exceeds 4 GByte or there are more than 65535 entries inside the archive and {@link #setUseZip64} is {@link Zip64Mode#Never}.
	 */
	 ...
	 *     on error
	 * @throws Zip64RequiredException
	 *     if the entry's uncompressed or compressed size exceeds 4 GByte and {@link #setUseZip64} is {@link Zip64Mode#Never}.
	 */
	 ...
	 * 
	 * @throws Zip64RequiredException
	 *     if the entry's uncompressed or compressed size is known to exceed 4 GByte and {@link #setUseZip64} is {@link Zip64Mode#Never}.
	 */
	 ...
	 *     on error
	 * @throws Zip64RequiredException
	 *     if the archive's size exceeds 4 GByte and {@link Zip64Mode #setUseZip64} is {@link Zip64Mode#Never}.
	 */
	 ...
	 *     on error
	 * @throws Zip64RequiredException
	 *     if the archive's size exceeds 4 GByte or there are more than 65535 entries inside the archive and {@link Zip64Mode #setUseZip64} is {@link Zip64Mode#Never}.
	 */
	...
	private Zip64ExtendedInformationExtraField getZip64Extra(ZipEntry ze) {
		...
		if (z64 == null) {
			/*
			 * System.err.println("Adding z64 for " + ze.getName() + ", method: " + ze.getMethod() + " (" + (ze.getMethod() == STORED) + ")" + ", raf: " + (raf != null));
			 */
