 ...
 * "@":  "&#064;" 
 */
package icu.etl.apache.ant.unrar.unpack;
...
import icu.etl.apache.ant.unrar.unpack.decode.Compress;
import icu.etl.apache.ant.unrar.unpack.ppm.BlockTypes;
import icu.etl.apache.ant.unrar.unpack.ppm.ModelPPM;
import icu.etl.apache.ant.unrar.unpack.ppm.SubAllocator;
import icu.etl.apache.ant.unrar.unpack.vm.BitInput;
import icu.etl.apache.ant.unrar.unpack.vm.RarVM;
import icu.etl.apache.ant.unrar.unpack.vm.VMPreparedProgram;
				...
				if (BlockLength <= WriteSize) {
					...
					if (ParentPrg.getGlobalData().size() > RarVM.VM_FIXEDGLOBALSIZE) {
						...
						for (int i = 0; i < ParentPrg.getGlobalData().size() - RarVM.VM_FIXEDGLOBALSIZE; i++) {
							Prg.getGlobalData().set(RarVM.VM_FIXEDGLOBALSIZE + i, ParentPrg.getGlobalData().get(RarVM.VM_FIXEDGLOBALSIZE + i));
						}
					}
					...
					if (Prg.getGlobalData().size() > RarVM.VM_FIXEDGLOBALSIZE) {
						...
						for (int i = 0; i < Prg.getGlobalData().size() - RarVM.VM_FIXEDGLOBALSIZE; i++) {
							ParentPrg.getGlobalData().set(RarVM.VM_FIXEDGLOBALSIZE + i, Prg.getGlobalData().get(RarVM.VM_FIXEDGLOBALSIZE + i));
						}
					} else {
					...
					while (I + 1 < prgStack.size()) {
						...
						if (pPrg.getGlobalData().size() > RarVM.VM_FIXEDGLOBALSIZE) {
							...
							for (int i = 0; i < pPrg.getGlobalData().size() - RarVM.VM_FIXEDGLOBALSIZE; i++) {
								NextPrg.getGlobalData().set(RarVM.VM_FIXEDGLOBALSIZE + i, pPrg.getGlobalData().get(RarVM.VM_FIXEDGLOBALSIZE + i));
							}
						}
						...
						if (NextPrg.getGlobalData().size() > RarVM.VM_FIXEDGLOBALSIZE) {
							...
							for (int i = 0; i < NextPrg.getGlobalData().size() - RarVM.VM_FIXEDGLOBALSIZE; i++) {
								pPrg.getGlobalData().set(RarVM.VM_FIXEDGLOBALSIZE + i, NextPrg.getGlobalData().get(RarVM.VM_FIXEDGLOBALSIZE + i));
							}
						} else {
