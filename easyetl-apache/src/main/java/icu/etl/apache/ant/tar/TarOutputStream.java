...
package icu.etl.apache.ant.tar;
...
import icu.etl.apache.ant.zip.ZipEncoding;
import icu.etl.apache.ant.zip.ZipEncodingHelper;
...
public class TarOutputStream extends FilterOutputStream {
	 ...
	 * 
	 * An archive consists of a series of file entries terminated by an end-of-archive entry, which consists of two 512 blocks of zero bytes. POSIX.1 requires two EOF records, like some other implementations.
	 * 
	 * @throws IOException
	 *     on error
	 */
	...
	public void putNextEntry(TarEntry entry) throws IOException {
		...
		boolean paxHeaderContainsPath = handleLongName(entryName, paxHeaders, "path", TarConstants.LF_GNUTYPE_LONGNAME, "file name");

		final String linkName = entry.getLinkName();
		boolean paxHeaderContainsLinkPath = linkName != null && handleLongName(linkName, paxHeaders, "linkpath", TarConstants.LF_GNUTYPE_LONGLINK, "link name");
	 ...
	 *     the name to write
	 * @param paxHeaders
	 *     current map of pax headers
	 * @param paxHeaderName
	 *     name of the pax header to write
	 * @param linkType
	 *     type of the GNU entry to write
	 * @param fieldName
	...
	private boolean handleLongName(String name, Map<String, String> paxHeaders, String paxHeaderName, byte linkType, String fieldName) throws IOException {
		...
		if (len >= TarConstants.NAMELEN) {
			...
			if (longFileMode == LONGFILE_POSIX) {
				...
				throw new RuntimeException(fieldName + " '" + name + "' is too long ( > " + TarConstants.NAMELEN + " bytes)");
			}
		}
		return false;
	}
