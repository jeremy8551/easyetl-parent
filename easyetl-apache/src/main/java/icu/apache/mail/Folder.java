...
import icu.apache.mail.event.ConnectionListener;
import icu.apache.mail.event.FolderEvent;
import icu.apache.mail.event.FolderListener;
import icu.apache.mail.event.MailEvent;
import icu.apache.mail.event.MessageChangedEvent;
import icu.apache.mail.event.MessageChangedListener;
import icu.apache.mail.event.MessageCountEvent;
import icu.apache.mail.event.MessageCountListener;
 ...
 * <p>
 *
 * The case-insensitive full folder name (that is, the full name relative to the default folder for a Store) <strong>INBOX</strong> is reserved to mean the "primary folder for this user on this server". Not all Stores will provide an INBOX folder, and not all users will have an INBOX folder at all times. The name <strong>INBOX</strong> is reserved to refer to this folder, when it exists, in Stores that provide it.
 * <p>
 *
 * A Folder object obtained from a Store need not actually exist in the backend store. The <code>exists</code> method tests whether the folder exists or not. The <code>create</code> method creates a Folder.
 * <p>
 *
 * A Folder is initially in the closed state. Certain methods are valid in this state; the documentation for those methods note this. A Folder is opened by calling its 'open' method. All Folder methods, except <code>open</code>, <code>delete</code> and <code>renameTo</code>, are valid in this state.
 * <p>
 *
 * The only way to get a Folder is by invoking the <code>getFolder</code> method on Store, Folder, or Session, or by invoking the <code>list</code> or <code>listSubscribed</code> methods on Folder. Folder objects returned by the above methods are not cached by the Store. Thus, invoking the <code>getFolder</code> method with the same folder name multiple times will return distinct Folder objects. Likewise for the <code>list</code> and <code>listSubscribed</code> methods.
 * <p>
 *
 * The Message objects within the Folder are cached by the Folder. Thus, invoking <code>getMessage(msgno)</code> on the same message number multiple times will return the same Message object, until an expunge is done on this Folder.
 * <p>
 *
...
public abstract class Folder {
	...
	public URLName getURLName() throws MessagingException {
		...
		if (fullname != null) {
			/*
			 * // We need to encode each of the folder's names. StringTokenizer tok = new StringTokenizer( fullname, new Character(separator).toString(), true);
			 * 
			 * while (tok.hasMoreTokens()) { String s = tok.nextToken(); if (s.charAt(0) == separator) encodedName.append(separator); else // XXX - should encode, but since there's no decoder... //encodedName.append(java.net.URLEncoder.encode(s)); encodedName.append(s); }
			 */
	...
	/**
	 * Returns a list of Folders belonging to this Folder's namespace that match the specified pattern. Patterns may contain the wildcard characters <code>"%"</code>, which matches any character except hierarchy delimiters, and <code>"*"</code>, which matches any character.
	 * <p>
	 *
	...
	/**
	 * Convenience method that returns the list of folders under this Folder. This method just calls the <code>list(String pattern)</code> method with <code>"%"</code> as the match pattern. This method can be invoked on a closed Folder.
	 *
	...
	/**
	 * Convenience method that returns the list of subscribed folders under this Folder. This method just calls the <code>listSubscribed(String pattern)</code> method with <code>"%"</code> as the match pattern. This method can be invoked on a closed Folder.
	 *
	 ...
	 *
	 * @exception FolderNotFoundException
	 *                if the implementation requires the folder to exist, but it does not
	...
	/**
	 * Returns true if this Folder has new messages since the last time this indication was reset. When this indication is set or reset depends on the Folder implementation (and in the case of IMAP, depends on the server). This method can be used to implement a lightweight "check for new mail" operation on a Folder without opening it. (For example, a thread that monitors a mailbox and flags when it has new mail.) This method should indicate whether any messages in the Folder have the <code>RECENT</code> flag set.
	 * <p>
	 *
	...
	/**
	 * Return the Folder object corresponding to the given name. Note that this folder does not physically have to exist in the Store. The <code>exists()</code> method on a Folder indicates whether it really exists on the Store.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * The <code>recurse</code> flag controls whether the deletion affects subfolders or not. If true, all subfolders are deleted, then this folder itself is deleted. If false, the behaviour is dependent on the folder type and is elaborated below:
	 * <p>
	 *
	 * <ul>
	 ...
	 *
	 * <li>Any messages within the folder are removed. Subfolders are not removed. The folder itself is not removed or affected in any manner. The delete() method returns true. And the exists() method on this folder will return true indicating that this folder still exists. <br>
	 * An appropriate FolderEvent is generated by the Store and this folder.
	 *
	 * <li>Any messages within the folder are removed. Subfolders are not removed. The folder itself changes its type from HOLDS_FOLDERS | HOLDS_MESSAGES to HOLDS_FOLDERS. Thus new messages cannot be added to this folder, but new subfolders can be created underneath. The delete() method returns true indicating success. The exists() method on this folder will return true indicating that this folder still exists. <br>
	 * An appropriate FolderEvent is generated by the Store and this folder.
	 * </ol>
	 * </ul>
	 *
	...
	/**
	 * Return the open mode of this folder. Returns <code>Folder.READ_ONLY</code>, <code>Folder.READ_WRITE</code>, or -1 if the open mode is not known (usually only because an older <code>Folder</code> provider has not been updated to use this new method).
	 *
	 * @exception IllegalStateException
	 *                if this folder is not opened
	 ...
	 * <p>
	 *
	 * The special flag <code>Flags.USER </code> indicates that this Folder supports arbitrary user-defined flags.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * This implementation returns -1 if this folder is closed. Else this implementation gets each Message in the folder using <code>getMessage(int)</code> and checks whether its <code>RECENT</code> flag is set. The total number of messages that have this flag set is returned.
	 *
	 ...
	 * <p>
	 *
	 * This implementation returns -1 if this folder is closed. Else this implementation gets each Message in the folder using <code>getMessage(int)</code> and checks whether its <code>SEEN</code> flag is set. The total number of messages that do not have this flag set is returned.
	 *
	 ...
	 * <p>
	 *
	 * This implementation returns -1 if this folder is closed. Else this implementation gets each Message in the folder using <code>getMessage(int)</code> and checks whether its <code>DELETED</code> flag is set. The total number of messages that have this flag set is returned.
	 *
	...
	/**
	 * Get all Message objects from this Folder. Returns an empty array if the folder is empty.
	 *
	 ...
	 * <p>
	 *
	 * Note that the specified Message objects <strong>must</strong> belong to this folder. Certain Folder implementations can optimize the operation of setting Flags for a group of messages, so clients might want to use this method, rather than invoking <code>Message.setFlags</code> for each Message.
	 * <p>
	 *
	 * This implementation degenerates to invoking <code>setFlags()</code> on each Message object. Specific Folder implementations that can optimize this case should do so. Also, an implementation must not abort the operation if a Message in the array turns out to be an expunged Message.
	 *
	 * @param msgs
	 ...
	 * @exception IllegalStateException
	 *                if this folder is not opened or if it has been opened READ_ONLY.
	 * @exception MessagingException
	 ...
	 * <p>
	 *
	 * Certain Folder implementations can optimize the operation of setting Flags for a group of messages, so clients might want to use this method, rather than invoking <code>Message.setFlags</code> for each Message.
	 * <p>
	 *
	 * The default implementation uses <code>getMessage(int)</code> to get each <code>Message</code> object and then invokes <code>setFlags</code> on that object to set the flags. Specific Folder implementations that can optimize this case should do so. Also, an implementation must not abort the operation if a message number refers to an expunged message.
	 *
	 * @param start
	 *            the number of the first message
	 * @param end
	 ...
	 * <p>
	 *
	 * This implementation just invokes <code>appendMessages()</code> on the destination folder to append the given Messages. Specific folder implementations that support server-side copies should do so, if the destination folder's Store is the same as this folder's Store. Also, an implementation must not abort the operation if a Message in the array turns out to be an expunged Message.
	 *
	 * @param msgs
	 *            the array of message objects
	 * @param folder
	...
	/**
	 * Expunge (permanently remove) messages marked DELETED. Returns an array containing the expunged message objects. The <code>getMessageNumber</code> method on each of these message objects returns that Message's original (that is, prior to the expunge) sequence number. A MessageCountEvent containing the expunged messages is delivered to any MessageCountListeners registered on the folder.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * After a message is expunged, only the <code>isExpunged</code> and <code>getMessageNumber</code> methods are still valid on the corresponding Message object; other methods may throw <code>MessageRemovedException</code>
	 *
	 ...
	 * <p>
	 *
	 * This implementation invokes <code>search(term, getMessages())</code>, to apply the search over all the messages in this folder. Providers that can implement server-side searching might want to override this method to provide a more efficient implementation.
	 *
	 * @param term
	 *            the search criterion
	 * @return array of matching messages
	 * @exception SearchException
	 *                if the search term is too complex for the implementation to handle.
	 * @exception FolderNotFoundException
	 *                if this folder does not exist.
	 * @exception IllegalStateException
	 *                if this folder is not opened.
	 ...
	 * <p>
	 *
	 * This implementation iterates through the given array of messages, and applies the search criterion on each message by calling its <code>match()</code> method with the given term. The messages that succeed in the match are returned. Providers that can implement server-side searching might want to override this method to provide a more efficient implementation. If the search term is too complex or contains user-defined terms that cannot be executed on the server, providers may elect to either throw a SearchException or degenerate to client-side searching by calling <code>super.search()</code> to invoke this implementation.
	 *
	 * @param term
	 *            the search criterion
	 * @param msgs
	...
	protected void notifyConnectionListeners(int type) {
		...
		/*
		 * Fix for broken JDK1.1.x Garbage collector : The 'conservative' GC in JDK1.1.x occasionally fails to garbage-collect Threads which are in the wait state. This would result in thread (and consequently memory) leaks.
		 * 
		 * We attempt to fix this by sending a 'terminator' event to the queue, after we've sent the CLOSED event. The terminator event causes the event-dispatching thread to self destruct.
		 */
	...
	/*
	 * A lock for creating the EventQueue object. Only one thread should create an EventQueue for this folder. We can't synchronize on the folder's lock because that would violate the locking hierarchy in some cases. For details, see the IMAP provider.
	 */
	...
	private void queueEvent(MailEvent event, Vector vector) {
		...
		/*
		 * Copy the vector in order to freeze the state of the set of EventListeners the event should be delivered to prior to delivery. This ensures that any changes made to the Vector from a target listener's method during the delivery of this event will not take effect until after the event is delivered.
		 */
