...
import icu.apache.mail.event.TransportListener;

/**
 * An abstract class that models a message transport. Subclasses provide actual implementations.
 * <p>
 *
 * Note that <code>Transport</code> extends the <code>Service</code> class, which provides many common methods for naming transports, connecting to transports, and listening to connection events.
 *
 * @author John Mani
 * @author Max Spivak
 * @author Bill Shannon
...
public abstract class Transport extends Service {
	...
	/**
	 * Send the message to the specified addresses, ignoring any recipients specified in the message itself. The <code>send</code> method calls the <code>saveChanges</code> method on the message before sending it.
	 * <p>
	 *
	 * @param msg
	 *            the message to send
	...
	private static void send0(Message msg, Address[] addresses) throws MessagingException {
		...
		/*
		 * More than one protocol. Have to do them one at a time and collect addresses and chain exceptions.
		 */
	 ...
	 * <p>
	 *
	 * Unlike the static <code>send</code> method, the <code>sendMessage</code> method does <em>not</em> call the <code>saveChanges</code> method on the message; the caller should do so.
	 *
	 * @param msg
