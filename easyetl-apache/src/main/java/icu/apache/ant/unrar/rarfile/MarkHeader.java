 ...
 */
public class MarkHeader extends icu.apache.ant.unrar.rarfile.BaseBlock {

    /** JDK日志输出接口 */
    private final static Logger log = Logger.getLogger(MarkHeader.class.getName());

    private boolean oldFormat = false;

    public MarkHeader(icu.apache.ant.unrar.rarfile.BaseBlock bb) {
        super(bb);
    }
    ...
    public boolean isValid() {
        ...
        if (!(getHeaderType() == icu.apache.ant.unrar.rarfile.UnrarHeadertype.MarkHeader)) {
            return false;
        }
        if (!(getFlags() == 0x1a21)) {
            return false;
...
//        logger.info("valid: " + isValid());
        log.log(Level.INFO, "valid: " + isValid());
    }
}
