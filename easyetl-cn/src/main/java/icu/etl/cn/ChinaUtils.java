...
public class ChinaUtils {
    ...
    public static boolean isIdCard(String idCard) {
        ...
        if (length == 18) { // 18位身份证
            ...
            return id18no == null ? false : isIdCard18(id18no);
        } else {
            return false;
        }
    }
    ...
    public static boolean isIdCard18(String idCard) {
        ...
        if (date.length() != 8 || Dates.testParse(date) == null) {
            return false;
        }
        ...
        for (int i = 0; i < idCard.length() - 1; i++) {
            try {
                j = j + Integer.parseInt(idCard.substring(i, i + 1)) * IDNO_CONSTANTS[i];
            } catch (Throwable e) {
                return false;
            }
        }
    ...
    public static boolean isChineseLetter(char c, CharsetEncoder encoder) {
        ...
        boolean block = (ub == Character.UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS //
                ...
                || JavaDialectFactory.getDialect().isChineseLetter(ub) //
        ...
        while (true) {
            ...
            if (numUnit > 0) {
                if ((numIndex == 9) && (zeroSize >= 3)) {
                    str.insert(0, "分角元拾佰仟万拾佰仟亿拾佰仟兆拾佰仟".charAt(6));
                }
                if ((numIndex == 13) && (zeroSize >= 3)) {
                    str.insert(0, "分角元拾佰仟万拾佰仟亿拾佰仟兆拾佰仟".charAt(10));
                }
    ...
    public static BigDecimal parseChineseNumber(CharSequence str) {
        if (StringUtils.isBlank(str)) {
            throw new IllegalArgumentException("parseChineseNumber(\"" + str + "\")");
        }
        ...
        if (array.length == 1) {
            ...
            throw new IllegalArgumentException("parseChineseNumber(\"" + str + "\") has mulity point!");
        }
    }
