...
public class PoolConnection implements InvocationHandler {
    ...
    private ConnectionProxy createProxy() {
        ...
        if (STD.out.isDebugEnabled()) {
            STD.out.debug(ResourcesUtils.getDatabaseMessage(42, this.getId(), this.conn.getClass().getName()));
        }
        return proxy;
    }
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        if (STD.out.isDebugEnabled()) {
            STD.out.debug(ResourcesUtils.getDatabaseMessage(35, this.getId(), this.conn.getClass().getName(), StringUtils.toString(method), StringUtils.toString(args)));
        }
    ...
    private synchronized void returnPool() {
        if (this.isIdle) {
            ...
            if (this.pool != null && !this.pool.isClose()) {
                ...
                if (STD.out.isDebugEnabled()) {
                    STD.out.debug(ResourcesUtils.getDataSourceMessage(11, this.toString()));
                }
