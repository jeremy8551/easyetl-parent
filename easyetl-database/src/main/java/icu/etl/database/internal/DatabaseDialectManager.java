...
public class DatabaseDialectManager {
    ...
    public synchronized void add(EasyetlContext context, BeanInfo beanInfo) {
        ...
        if (beanInfo instanceof BeanInstance) {
            BeanInstance instance = (BeanInstance) beanInfo;
            dialect = (instance.getBean() == null) ? context.createBean(beanInfo.getType()) : instance.getBean();
            instance.setBean(dialect);
        } else {
    ...
    public Class<?> getDialectClass(String name, String major, String minor) {
        BeanInfoList list = this.map.get(name);
        if (list == null) {
        ...
        if (StringUtils.isNotBlank(major) || StringUtils.isNotBlank(minor)) {
            ...
            BeanInfoRegister beanInfo = buffer.getBeanInfo();
            if (beanInfo != null) {
