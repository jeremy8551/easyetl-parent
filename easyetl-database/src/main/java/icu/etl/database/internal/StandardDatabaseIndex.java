    ...
    public int compare(DatabaseIndex index1, DatabaseIndex index2) {
        ...
        if (index1 == null || index2 == null) {
            if (STD.out.isDebugEnabled()) {
                STD.out.debug((index1 == null) + " != " + (index2 == null));
            }
        ...
        if (nameList1.size() != nameList2.size()) { // 判断字段数
            if (STD.out.isDebugEnabled()) {
                STD.out.debug(ResourcesUtils.getDatabaseMessage(39, index1.getName(), index2.getName(), nameList1.size(), nameList2.size()));
            }
            return nameList1.size() - nameList2.size();
        }
        if (icsList1.size() != icsList2.size()) { // 判断字段数
            if (STD.out.isDebugEnabled()) {
                STD.out.debug(ResourcesUtils.getDatabaseMessage(39, index1.getName(), index2.getName(), icsList1.size(), icsList2.size()));
            }
        ...
        for (int i = 0; i < nameList1.size(); i++) { // 判断字段名
            ...
            if (c != 0) {
                if (STD.out.isDebugEnabled()) {
                    STD.out.debug(ResourcesUtils.getDatabaseMessage(40, index1.getName(), index2.getName(), (i + 1), nameList1.get(i), nameList2.get(i)));
                }
                return c;
            }
        }
        ...
        for (int i = 0; i < icsList1.size(); i++) { // 判断字段排序方式
            ...
            if (c != 0) {
                if (STD.out.isDebugEnabled()) {
                    STD.out.debug(ResourcesUtils.getDatabaseMessage(41, index1.getName(), index2.getName(), (i + 1), icsList1.get(i), icsList2.get(i)));
                }
                return c;
            }
        }
